...
class OperatorRegressions(unittest.TestCase):
    ...
    def run_execute(self, action, kind, live_change=None, deployed=None, target=None, accept=False, classification=True,
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            if planned_web_cors is not None:
                templates['WebEdge']['Resources']['WebBucket12880F5B']={
                    'Type':'AWS::S3::Bucket','Properties':{'CorsConfiguration':planned_web_cors}}
                live['WebEdge']['Resources']['WebBucket12880F5B']={'Type':'AWS::S3::Bucket','Properties':{}}
    ...
    def test_stateful_guard_allows_only_the_approved_web_bucket_cors_addition(self):
        cors={'CorsRules':[{'AllowedOrigins':['https://d54awmnmi4c3z.cloudfront.net'],
                            'AllowedMethods':['PUT'],'AllowedHeaders':['content-type'],'MaxAge':300}]}
        bucket_id='WebBucket12880F5B'
        old={'Resources':{bucket_id:{'Type':'AWS::S3::Bucket','Properties':{'VersioningConfiguration':{'Status':'Enabled'}}}}}
        planned=json.loads(json.dumps(old))
        planned['Resources'][bucket_id]['Properties']['CorsConfiguration']=cors
        with tempfile.TemporaryDirectory() as d, patch.object(ops,'aws',return_value={'TemplateBody':old}):
            ...
            for changed in ('origin','versioning','existing-cors'):
                with self.subTest(changed=changed):
                    ...
                    if changed=='origin':
                        variant['Resources'][bucket_id]['Properties']['CorsConfiguration']['CorsRules'][0]['AllowedOrigins']=['https://example.invalid']
                    elif changed=='versioning':
                        variant['Resources'][bucket_id]['Properties']['VersioningConfiguration']['Status']='Suspended'
                    else:
                        previous['Resources'][bucket_id]['Properties']['CorsConfiguration']={'CorsRules':[]}
    ...
    def test_post_upload_cors_exception_rejects_a_different_bucket_identity(self):
        cors={'CorsRules':[{'AllowedOrigins':['https://d54awmnmi4c3z.cloudfront.net'],
                            'AllowedMethods':['PUT'],'AllowedHeaders':['content-type'],'MaxAge':300}]}
        old={'Resources':{'OtherBucket':{'Type':'AWS::S3::Bucket','Properties':{}}}}
        planned={'Resources':{'OtherBucket':{'Type':'AWS::S3::Bucket','Properties':{'CorsConfiguration':cors}}}}
        with tempfile.TemporaryDirectory() as d:
            root=Path(d);assembly=root/'assembly';assembly.mkdir()
            bodies={name:{'Resources':{}} for name in ops.PROTECTED+['Services']}
            for name,body in bodies.items():
                (assembly/f'NullnullStg{name}.template.json').write_text(json.dumps(body))
            (assembly/'NullnullStgWebEdge.template.json').write_text(json.dumps(planned))
            bodies['WebEdge']=old
            with patch.object(ops,'aws',return_value={'TemplateBody':old}):
                with self.assertRaisesRegex(ops.OpsError,'stateful-change'):
                    ops.guard_stateful('WebEdge',assembly)
            self.assertIn('template-changed-WebEdge',ops.preserve_open_template_findings(root,bodies))
