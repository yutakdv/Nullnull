...
2. BA-xxx-Tn acceptance는 구현 PR에서 실제 test class/함수 이름과 report 경로를 카드에 적고 `backend-plan.json` status를 올린다.
   ...
   **test ID는 자기가 증명하는 절의 카드를 가리킨다. 가장 가까운 카드의 ID를 빌리지 않는다**([#195](https://github.com/yutakdv/Nullnull/issues/195)). `backend-plan.json`은 카드에만 `tests[]`를 두므로 CON·결정·이슈 단위의 cross-cutting 작업은 자기 ID를 가질 수 없고, 그래서 이웃 카드의 ID를 빌리게 된다. 빌린 이름은 두 방향으로 틀린다. 빌려준 카드가 실제보다 잘 덮여 보이고(`BA-003-T1`을 단 7건 중 그 절을 재는 것은 `HealthScopeIT`의 3건뿐이었다), 진짜 소유 작업에는 증거가 남지 않는다. 그 속성을 소유하는 카드에 절을 더하고 그 ID를 단다 — 공통 HTTP 선언은 `BA-003`이 소유한다. 소유 카드가 정말 없는 첫 사례가 나오면 그때 plan에 카드가 아닌 단위를 들인다(최소 설계는 #195를 닫는 코멘트에 있다).
