...
7. **검사를 넣었다와 검사가 발화한다는 다른 확인이다. 새 검사를 넣는 PR에서 둘 다 한다.**
   ...
   **거울상은 시끄럽게 틀리는 가드이고, 그쪽이 싸다.** 같은 가드가 나중에 배치를 막았는데 거짓 양성이었다 — 보호 대상은 **오너의 hunk**인데 정규식이 **파일 이름**을 걸었고, 그 파일에 든 것은 무관한 문단이었다. 조용한 오류는 세 번의 push를 그냥 통과시켰고 시끄러운 오류는 5분을 멈추고 **실제 불변식**을 찾게 했다(`git diff -- <파일> | grep -c '^[+-][^+-]'`가 8인가). 가드를 쓸 때는 **무엇이 보호 단위인지**를 먼저 적는다 — 파일·경로·hunk는 다른 것이고, 규칙 6의 *"선언은 소유가 아니다"* 가 사람에 대해 적은 것이 정규식에서도 그대로 난다.
