...
export interface paths {
    ...
    "/posts/images/uploads": {
         ...
         *     prefix. The signature is bound to the content type and length declared here. The server
         *     compares the declared length and SHA-256 with the uploaded bytes before it publishes
         *     anything; a changed file is refused even if it is a valid image of the same size. The ticket is
         *     single use and short lived, and the offered
         ...
         *     A completed retry with the same Idempotency-Key and declaration replays the same
         *     ticket, including its original expiry; use a new key after that expiry. Reusing the
         *     key with a different declaration returns 409.
         */
    ...
    "/posts": {
         ...
         *     A completed retry with the same Idempotency-Key and body instead replays the original
         *     201 and postId without reading or publishing the image again. Reusing that key with a
    ...
    "/places/crowd-forecasts/query": {
         ...
         *     `unavailableReason` is a token from CrowdSeries' list, never display text. `to` is
    ...
    "/trips/{tripId}/candidates": {
         ...
         *     A new POST-source save requires a currently PUBLISHED source post. DRAFT, HIDDEN
         *     and unknown post IDs all return NOT_FOUND without creating a candidate or source.
         *     A replay of an already committed idempotency key remains a replay, not a new save.
         */
...
export interface components {
    schemas: {
        ...
        CrowdMetric: {
             ...
             *     never derived from `value`. A stage a source stored without a reviewed mapping onto this
             *     scale is served as null with `SCHEMA_DRIFT` in `provenance.qualityFlags`, rather than passed
             *     through as a stage nobody approved, and that is still what happens to every source but the
             *     one named below. KTO is one of them: it publishes a relative index with no stages at all.
             *
             *     `SEOUL_CITYDATA` is the first source with a reviewed mapping - owner decision A-060,
             *     2026-09-20. **It publishes four steps, and they are placed on cells "1" to "4"; it never
             *     sends "5".** The empty top cell is deliberate: Seoul's top step is "over 100% with no
             *     ceiling", so calling it the maximum of this scale would assert something the provider never
             *     said. **Its thresholds are percentages of that area's own past average**, not of a capacity
             *     and not of anything comparable between areas, so two areas both at "3" are each three
             *     quarters of their own normal and that says nothing about which is busier - comparing them
             *     is what invariant 8 forbids.
             *
             *     The words for each stage are Frontend copy, as before - but they cannot be one fixed list
             *     of five, because a source may fill only part of the scale and publish its own words for the
             *     steps it does fill. Read `provenance.source` to know which mapping produced the cell, and
             *     do not describe a reading as "Nth of five" unless that source actually publishes five.
             */
        ...
        CrowdSeries: {
            ...
            /**
             * @description Why the series has no points, as a token Frontend maps to its own copy - never display text.
             *     Null unless `state` is UNAVAILABLE. `NO_COVERAGE`: nothing stored covers the window; most
             *     places answer this and it is not an outage, so it should not read like one.
             *     `PLACE_UNAVAILABLE`: the requested id cannot be read (unknown, not active, or without
             *     coordinates); only queryPlaceCrowdForecasts sends it, where getPlaceCrowdForecast answers 404.
             *     `x-extensible-enum` lists every token the server sends today. The list may grow, and a new
             *     token is added there when it starts being sent, so render one you do not know as unavailable;
             *     never print it.
             */
        ...
        LiveArea: {
             ...
             *     withheld, it is unknown. Areas without a reviewed centroid remain in the list
             *     and have no map marker. The map may show a selected place using its own
             *     PlaceDetail.location; that place position is not the area's centroid.
             */
