...
if (app.node.tryGetContext("phase") === "foundation") {
  ...
  createStacks(app, { account, bootstrapOnly: true, webDirectory: "", coversDirectory: "",
  ...
  const coversDirectory = app.node.tryGetContext("coversDirectory");
  if (!manifest || !webDirectory || !coversDirectory)
    throw new Error("Explicit releaseManifest, webDirectory and coversDirectory required");
  ...
  createStacks(app, {account, release, webDirectory, coversDirectory});
}
