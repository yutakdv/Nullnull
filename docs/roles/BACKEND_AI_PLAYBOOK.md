...
- `BA-002-T5`: 앞선 호출이 prelude를 도는 동안 같은 key로 온 호출은 prelude도 command도 돌리지 않고 앞선 호출의 응답을 replay한다 — `IdempotencyGuardPreludeIT.theKeyIsHeldWhileThePreludeRuns`(두 번째 호출이 예약 행에서 막힌 것을 `pg_blocking_pids`로 본 뒤 첫 호출을 푼다 — 두 번째 호출은 owner 행을 쥔 채 예약을 읽으므로 첫 호출이 그 사이에 끝날 수 없다)
- `BA-002-T6`: prelude가 있는 command의 완료 기록은 lease가 아니라 idempotency 보존기간 동안 남는다 — `IdempotencyGuardPreludeIT.aCompletedPreludeCommandIsRetainedLikeAnyOther`
- `BA-002-T7`: 실패한 prelude는 예약을 풀어 같은 key가 곧바로 다시 실행된다 — `IdempotencyGuardPreludeIT.aFailedPreludeFreesTheKey`
...
**`T5`~`T12`는 #340이다.** guard의 두 번째 진입점 `execute(…, Prelude, …)`는 외부 호출(prelude)을 command 앞에 둬야 하는 command를 위한 것이다. 예약을 먼저 짧은 transaction으로 커밋하고, 그 예약을 쥔 호출만 transaction 없이 prelude를 돌린 뒤 문서화된 순서(owner → 예약 → command가 잠그는 것)로 command를 실행한다. 그래서 prelude 동안 key는 다른 transaction이 볼 수 있는 미완료 행으로 잡혀 있다 — 단일 단계 guard는 이 상태를 남기지 않는다. 그 행의 `expires_at`이 lease다(prelude bound + lock timeout 다섯, migration 없음). lease가 끝난 예약은 기존 규칙(만료 행은 빈 슬롯)대로 다음 호출이 넘겨받는다(`T9`). lease는 liveness 상한이고 정확성은 command transaction이 owner 잠금 아래에서 예약이 아직 자기 것인지 다시 보는 데 있다(`T12`). 완료되면 `expires_at`을 보존기간으로 바꾼다(`T6`). 실패는 예약을 즉시 푼다 — 해제 지점이 prelude 뒤와 command 뒤 둘이라 절도 둘이다(`T7`·`T8`). 단일 단계 경로는 남의 미완료 예약 위에서 command를 돌리지 않는다(`T10`). 쓰는 곳은 지금 `decideOptimization` 하나다([BA-052](#ba-052)의 `T23`~`T25`).
...
**`T13`~`T15`는 #340이다.** prelude가 있는 command([BA-002](#ba-002)의 `T5`)의 claim도 owner 행 경합을 만난다 — 같은 owner의 다른 command transaction이 그 행을 쥔 동안이다. 이 카드가 정한 공개 계약(경합은 흡수하고, 소진은 `INTERNAL_ERROR`)을 그 경로에서도 지킨다. claim이 lock timeout으로 끝나면 key가 사용 중인 것으로 읽고 자기 bound 안에서 다시 claim한다(`T14`). 소진은 owner 행이 그 bound를 넘게 잡혀 있을 때뿐이고 그때도 아무것도 예약하지 않는다(`T13`). 살아 있는 예약이 key를 쥐고 있는 동안에는 소진하지 않는다 — 기다림은 그 예약의 lease에 묶이고, 새 보유자가 key를 넘겨받으면 그 보유자의 lease로 다시 잡힌다(`T15`).
...
PM-019 검토 결과: operation별 401/`default`/403 선언 보강은 이 slice에서 하지 않았다. 생성 TypeScript client가 바뀌어 Frontend가 첫 green `docker-integration` 도중에 재생성을 강요받기 때문이다. [IMPLEMENTATION_PLAN](../engineering/IMPLEMENTATION_PLAN.md#공동-실행-id)의 `CON-006`으로 등록했고 그 뒤 #170에서 착수했다. **429는 그 목록에서 빠졌다** — A-025로 `apps/api`에 producer가 없으므로 새로 열거하지 않고, 기존 선언 **넷**(`issueCsrfToken`·`getDeletionRequest`·`listOptimizationHistory`·`markAllNotificationsRead`)은 edge가 낼 수 있으며 응답 삭제가 breaking이라 남긴다. 계약에서 `"429":`를 grep하면 여섯이 나오는데 그중 둘(`createOptimization`·`decideOptimization`)은 **응답 선언이 아니라 `x-error-codes` 항목**이다 — 세는 대상이 다르다. 문서 전역 규칙(`x-nullnull-common-contract`의 `protectedErrors`·`rateLimitErrors`·`bounds`)은 이미 계약에 있으므로 이 slice는 그 약속을 server가 지키게 만드는 쪽만 했다.
...
- `BA-005-T4`: handler의 dead-letter hook이 불리는 job의 dead letter는 그 hook과 한 commit으로만 기록된다 — `JobWorkerIT.aDeadLetterRollsBackWithAHookThatFails`. queue가 payload를 읽을 수 없는 행은 hook 없이 끝난다(`T7`·`T10`).
...
- `BA-005-T6`: handler가 필요한 식별자를 읽을 수 없는 payload의 최적화 job은 재시도 없이 첫 attempt에서 dead letter가 된다 — `OptimizeGatewayFailureIT.anUnreadablePayloadIsNotRetried`. T5와 T6은 같은 규칙을 handler별로 나눈 것이다. 생산자가 둘(`DeleteOwnerDataHandler.requiredUuid`·`OptimizeItemHandler.runId`, 지금 있는 `JobHandler` 전부)이라 한 ID로 묶으면 어느 한쪽 test만으로 그 ID가 충족된다. 다음 attempt도 같은 payload를 읽으므로 둘 다 `INVALID_JOB_PAYLOAD`를 non-retryable로 던지고, worker가 그 attempt에서 끝낸다. 전에는 상한까지 back-off를 거치며 같은 실패를 되풀이했다(측정: 상한 3인 삭제 job이 attempt 3에서 FAILED). cause는 붙이지 않는다 — 그 message가 payload 값을 worker의 WARN 줄로 옮긴다. queue 자신이 읽을 수 없는 payload(`JobPayload`의 식별자 모양을 벗어난 값, 객체가 아닌 JSON)는 이 절 밖이고 `BA-005-T7`~`T10`이 다룬다. 그런 행은 handler에 닿지 않는다.
- `BA-005-T7`: 저장된 payload를 읽을 수 없는 job은 handler 없이 첫 claim에서 INVALID_JOB_PAYLOAD dead letter가 된다 — `JobWorkerIT.anUnreadablePayloadEndsOnItsFirstClaimWithoutItsHandler`
- `BA-005-T8`: payload를 읽을 수 없는 job은 같은 type의 뒤 job claim을 막지 않는다 — `JobWorkerIT.anUnreadablePayloadDoesNotHoldUpTheQueue`
- `BA-005-T9`: abandoned sweep은 payload를 읽을 수 없는 행이 섞여도 그 type의 abandoned 행을 모두 LEASE_EXPIRED로 끝낸다 — `JobAbandonedLeaseIT.theAbandonedSweepEndsAnUnreadableRowWithTheRest`
- `BA-005-T10`: abandoned sweep은 payload를 읽을 수 없는 행의 dead-letter hook을 부르지 않는다 — `JobAbandonedLeaseIT.theHookIsNotGivenAnInventedPayload`
...
- 검증함(queue가 읽을 수 없는 payload, `BA-005-T7`~`T10`): 전에는 claim과 abandoned sweep의 mapper가 statement 안에서 throw해 statement 전체가 rollback됐다. 그래서 그 행이 같은 type의 뒤 job을 영원히 막았고, sweep은 그 type의 abandoned 행을 하나도 끝내지 못했다. 세 test(T7·T8·T9)는 수정 전에 모두 빨갰다(terminal status에 닿지 못함). T7의 로그 검사는 root logger의 message와 cause에서 canary를 찾는다. 그중 하나는 `JobPayload`가 거부할 때 message에 인용하는 key다. 이제 mapper는 "읽을 수 없음"을 표시하고 worker가 그 job을 handler·hook 없이 `INVALID_JOB_PAYLOAD`로 끝내며, sweep은 그 행도 `LEASE_EXPIRED`로 끝내되 hook은 부르지 않는다. 변이 일곱이 각자 발화했다: claim mapper throw → T7·T8, sweep mapper throw → T9·T10, worker가 표시 무시 → T7·T8, sweep이 hook 호출 → T10만, dead letter 대신 retry → T7·T8, 버린 cause를 log에 남김 → T7만, re-take alarm을 unreadable 분기 뒤로 → re-take test만(앞의 셋과 retry 변이도 그 test를 빨갛게 한다).
...
- 남은 위험, handler slice가 책임진다: `JobUnitOfWorkGuard`는 handler thread에서 시작된 transaction만 본다. 짝이 되는 `ArchitectureRulesTest.jobHandlersNeverTouchTheDatabaseDirectly`는 **직접 참조만** 금지하므로, handler가 `@Transactional`이 전혀 없는 평범한 협력 class를 통해 `jdbc.sql("INSERT ...").update()`를 실행하면 rule도 guard도 통과하고 lease 밖에서 commit된다(측정함). 한 단계 건너뛴 협력자까지 막는 검사는 아직 없다. 이 줄은 원래 *첫 handler를 붙이는 [BA-012](#ba-012)가 그것을 slice acceptance로 잡는다* 고 미래형으로 적혀 있었는데, **BA-012는 이미 병합됐고 그 acceptance는 만들어지지 않았다.** 코드로 확인한 현재 상태는 이렇다 — `DeleteOwnerDataHandler.handle`의 쓰기 **네 자리가 전부** `context.transactional(...)` 안에 있다(`markRunning`, eraser 각각의 `erase`, `markCompleted`, 실패 경로의 `markFailed`). 즉 **속성은 참이고, 그것을 지키는 검사는 없다.** `jobHandlersNeverTouchTheDatabaseDirectly`는 여전히 직접 참조만 막으므로, 다음 handler(B03 collector, B06 optimization)가 평범한 협력 class를 하나 끼우면 rule도 guard도 통과한 채 lease 밖에서 commit된다. **다음 handler를 붙이는 slice가 이 검사를 함께 만든다** — 가장 싼 형태는 `JobContext.transactional` 호출을 기록하는 fake로 handler를 돌려 모든 store 호출이 그 안에서 일어났는지 보는 것이고, ArchUnit으로는 문장 수준을 볼 수 없어 잡히지 않는다.
...
rc.24 staging 실측(`v0.1.0-rc.24`, main `59ca9269`, [#305 Wave 4](https://github.com/yutakdv/Nullnull/issues/305#issuecomment-5847141062)). **제출 release 증거가 아니다.** 제출 release는 R5(#185 수정 배포)가 되고, 이 증거는 release별이라 제출 release에서 다시 만든다. rc.6 실측(`backend-plan.json`의 handoff)과 같은 등급의 기록이다. 위 "(T3 전)" 목록은 그 시점의 기록이다. rc.24 시점의 staging은 `infra/staging.config.json`의 `catalogPublicEnabled: true`로 catalog를 공개했고, rc.24 flows의 `catalog.search`는 200이었다.

- 실제 호출: 오너 셸의 `kto-smoke`가 낙산공원(`129501/12`)을 불렀고, [SUBMISSION_RUNBOOK ①](../contest/SUBMISSION_RUNBOOK.md#제출-release-대조-검사)의 성공 줄 `actual_call=verified release=v0.1.0-rc.24 environment=staging operation=KorService2/detailCommon2`가 나왔다. 보고서의 호출은 1건이고 `outcome=OK`, `gitSha`는 release SHA `59ca9269`다. 같은 release의 호출 목록에서 `KTO_KOR_SERVICE_2` detailCommon2는 3건이고, 마지막 호출이 이 smoke다(보고서의 관측 시각과 같은 초).
- 장소 선택: Wave 4 명령표는 서울숲(`128611`)을 적었는데 `129501`로 바꿔 불렀다. #305 코멘트가 든 이유는 "catalog 장소는 schedule이 detail snapshot을 갱신해 신선하므로 smoke가 호출 없이 `CACHED_SNAPSHOT`으로 답했을 것"이다. **이 이유는 release 코드와 맞지 않는다.** `59ca9269`의 `KtoSmokeMain`은 365일 뒤에도 신선한 snapshot만 받는다(`FORCE_HORIZON`). KorService2 snapshot의 수명은 7일이라 저장된 snapshot은 그 조건을 만족할 수 없고, smoke는 항상 호출한다(javadoc: *"It always calls"*). 또 `129501`은 runbook이 정한 "마지막 호출이 통과한 장소"가 아니다. 그 규칙은 거절된 호출이 `KTO_KOR_SERVICE_2` source를 격리하기 때문에 있다. 이번 호출은 통과했으므로 격리는 없었다. #305 코멘트에 따르면 `129501`은 catalog 밖이고, 부르기 전에 공식 KTO 사이트에서 contentId와 contentType을 확인했으며, 공개 catalog에는 적재하지 않았다(이 카드는 대조하지 않았다). 그 보고대로라면 이 호출의 snapshot은 공개 응답에 나가지 않는다.
- 공개 응답 provenance: 같은 release를 배포한 직후 `staging_flows=pass checks=27 failures=0`이다. `catalog.search`가 `items=1`로 통과했으므로 `catalog.provenance`가 돌았고, 실패가 0이라 통과했다. 그 check 줄은 기록 파일에 옮겨지지 않아 `staging-flows.mjs` 코드와 요약 줄로 판정했다. 이 check는 검색 첫 장소의 `sourceAttribution`에 source·attribution·registry version이 있는지만 본다.
- **"연결"은 rc.24 증거로 채워지지 않았다.** 연결이 어디까지 가능한지부터 적는다(`59ca9269` 코드). 공개 read path(`JdbcCatalogPlaceQuery`)는 `kto_place_snapshots`를 읽지 않는다. 공개 행(`places`·`place_external_refs`·`place_localizations`)에는 snapshot id가 없고, revision pin과 첫 적재 시각(`verified_at`)만 있다. 이미 매핑된 장소는 ingest가 다시 쓰지 않는다(`KtoSnapshotCatalogIngest`, `createIfAbsent`의 INSERT뿐). 그래서 공개 값이 특정 호출에서 나왔다는 연결은 이미 공개된 장소에서는 구조상 만들 수 없다. 기존 도구로 보일 수 있는 것은 식별자 연결이다. 호출의 (source, contentId)가 공개 `getPlace`의 `externalRefs`와 같고, 그 장소가 flows의 `catalog.provenance`가 본 첫 검색 결과와 같은 레코드인지를 본다. rc.24는 호출 장소(`129501`)가 공개 catalog 밖이라(#305 보고) 식별자 연결도 없다. 두 줄은 같은 release라는 것 말고는 이어지지 않는다. rc.6의 "연결됐다"(`backend-plan.json` handoff)도 같은 check에 기댄다. 그때 smoke는 서울숲(`128611`)이었고 flows의 기본 검색어는 경복궁이라, 같은 레코드였는지는 그 기록만으로 알 수 없다.
- 이 기준에서 [ENVIRONMENT.md](../operations/ENVIRONMENT.md)가 적은 공개 flag의 전제(T3 staging 증거)가 충족됐는지는 이 카드가 정하지 않는다. rc.24 시점의 staging은 이미 켜져 있었고, 그 판단은 오너 몫으로 남긴다.
- mock-only 쪽: rc.24에서 새로 잰 것은 없다. 위 성공 줄은 이 release의 실제 보고서가 판정을 통과했다는 기록이다. 같은 줄은 `--require-verified` 유무와 상관없이 찍힌다(그 옵션은 보고서가 없을 때만 다르게 동작한다). mock·replay·fixture 보고서를 거절하는 논리는 `test_actual_call_evidence.py`가 합성 입력으로 잰다([BA-073](#ba-073)의 `T2` 항목).
- 보고서는 release bucket `evidence/`와 R4 배포 worktree의 `.artifacts/submission/v0.1.0-rc.24/`(gitignore)에 있다.
- status는 `in-progress` 그대로다. T3는 JUnit 이름을 가질 수 없고(위), "연결"도 채워지지 않았다.
...
- `BA-051-T29`: READY run의 fingerprint는 run이 저장한 기한으로 계산된다
- `BA-051-T30`: 재시도는 첫 시도가 동결한 기한을 유지한다
- `BA-051-T31`: 기한이 지난 뒤 끝나는 run은 failure code가 아니라 EXPIRED로 기록된다
- `BA-051-T32`: store는 기한 시각 이후의 run을 READY로 쓰지 않는다
...
- `BA-052-T23`: 같은 Idempotency-Key로 동시에 온 두 APPLY는 apps/ai에 한 번 묻고 둘 다 그 한 결정으로 답한다
...
- `BA-071-T3`: staging 배포는 release manifest에 묶인 산출물 전체와 승인된 CDK assembly만 실행한다
...
- 증명(T7): staging API 로그 그룹에 다섯 이름의 `ops.alarm name=<NAME> drill=BA-072-T7` 줄을 넣어 두 번 쟀다. **첫 drill이 결함을 찾았다** — 다섯 alarm이 모두 `OK → ALARM`으로 갔는데 SNS action이 전부 `Failed to execute action`이었다. `enforceSSL`이 만든 topic policy(Deny 한 줄)가 기본 policy를 **교체**해 CloudWatch 서비스 주체의 Allow가 사라져 있었고, 그때까지 어떤 alarm도 사람에게 닿은 적이 없었다(구독 script의 test 메일은 IAM 주체라 도착했다). 이 계정 alarm만 publish하는 Allow를 더해(`CloudWatchAlarmsPublish`, topic policy test가 고정) release `c083f1e`로 배포한 뒤 같은 drill을 다시 돌리자 다섯이 `OK → ALARM` 뒤 `Action Successfully executed`였고 **primary 수신자가 메일을 받았다**(오너 확인). metric filter 다섯은 배포된 문구를 그대로 인용하고 metric이 그 분에 1을 기록한다. A-043에 따라 secondary 수신자는 두지 않는다.
...
- 증명(`T4`, 게이트): 막는 장치가 release의 사슬 안에서 돌고, 이 절의 ID로 집계된다. `apps/web/e2e/attribution.integration.spec.ts`(#382)의 "BA-073-T4 every place the trip response credits shows that credit on screen"이 실제 API의 여행 응답이 출처를 준 장소마다 그 카드가 서버의 출처 문구를 그대로 그리는지 본다. `docker-integration`의 E2E에서 돌고 E2E JUnit은 `--e2e-junit-dir`로 집계된다. main `63d981f8`의 Docker integration run 36013007001이 성공이다. 여행 화면의 장소 출처를 지우면 이 test만 빨개진다는 것은 #382의 측정이다([#53 코멘트](https://github.com/yutakdv/Nullnull/issues/53#issuecomment-5818413401), 이 카드에서 다시 재지 않았다). 자리 단위(타입 기반) 정적 검사 `apps/web/src/shared/ui/__tests__/attribution-coverage.test.ts`(FE-603-T4)도 같은 게이트의 web suite에서 돈다. **이 test가 보는 것은 여행 응답이 부른 장소뿐이다.** rc.23 확인 중에 여행 생성 수동 단계의 장소 행이 KTO 출처 없이 이름만 그리는 것이 나왔다(같은 코멘트). 그 뒤 #389(merge `ce7a99b8`)가 장소를 보여 주는 단위마다 장소·텍스트 출처를 그리고 출처 검사를 자리 단위로 바꿨다. `BA-073-T4` 자체가 보는 범위는 여전히 여행 응답이 부른 장소뿐이다.
- 증명(`T5`, 게이트): 막는 장치가 release의 사슬 안에서 돌고, 이제 이 절의 ID로 집계된다. `apps/web/e2e/location-off.spec.ts`(FE-603-T1)의 testcase 제목이 `FE-603-T1 BA-073-T5`를 단다. `screens.ts`의 화면마다 한 건씩 geolocation 호출(API를 감싸 기록하고 throw한다)·JS dialog(`page.on('dialog')`)·나가는 요청의 좌표를 보고, 전체 한 건이 `navigator.permissions.query`로 geolocation을 묻는 화면이 없음을 단언한다("no screen registers a geolocation permission at all"). 브라우저 권한 prompt는 JS dialog가 아니라 `page.on('dialog')`로 오지 않는다(Playwright `Dialog.type()`은 alert·beforeunload·confirm·prompt뿐이다). 권한 요청을 실제로 막는 것은 geolocation wrapper다. `docker-integration`의 E2E에서 돌고 E2E JUnit은 `--e2e-junit-dir`로 집계된다. `main` 트리는 화면이 22개라 23건이다(소스로 셈). #360 로컬 게이트(`backend` 트리, 화면 21개)에서는 22건이 전부 통과한 것을 report로 쟀다.
...
**#338 표지 회수 경계.** `withdraw-post`는 먼저 DB에서 게시물을 HIDDEN으로 commit하고, 반환된 `USER_UPLOAD` 표지 한 URL이 배포된 공개 origin의 `covers/user/<uuid>.jpg|png`인지 검증한 뒤 정확한 S3 key의 모든 객체 버전·삭제 마커를 제거한다. 다른 prefix·인접 key는 삭제하지 않는다. OpsTask 역할만 `s3:ListBucketVersions`와 `s3:DeleteObjectVersion`을 사용자 표지 prefix에 받으며 API 역할에는 삭제 권한이 없다. 부분 실패는 성공 로그 대신 `cover_cleanup=pending`으로 종료하고 같은 승인 게시물 ID 재시도로 남은 버전만 제거한다. 기존 후보의 `sources[].postId`와 이미 commit된 idempotency replay는 남고, 회수 뒤 새 POST 출처 저장은 PUBLISHED 검사에서 거절된다.

**#338 캐시와 완료 증거.** 새 업로드 표지는 `Cache-Control: no-store`로 발행하고 service worker는 `/covers/user/`를 캐시에 넣지 않는다. 기존 객체 metadata와 이미 브라우저·기기·제3자가 가진 사본은 강제로 회수할 수 없다. 테스트는 `PublishedCoverRemovalTest`(정확한 key·전버전·마커·부분 실패 재시도), `PostWithdrawMainTest`, `WithdrawPostTaskRegressions`, infra IAM synth, `S3ObjectStorageTest`, `CandidateIT`다. 운영 배포·실제 S3 canary·공개 URL 검증 전까지 [#338](https://github.com/yutakdv/Nullnull/issues/338)은 완료로 표시하지 않는다.
...
5. 서울 관측은 API 작업의 5분 주기 원자적 claim으로 적재하고 장소 연결은 소유자가 해시를 승인한 매핑 계획으로 적재한다
...
- `BA-091-T19`: 오래된 승인 계획은 더 새로운 매핑 검토를 되돌리지 않는다
- `BA-091-T20`: 새 승인 계획은 이전 구역 연결을 원자적으로 교체한다
- `BA-091-T21`: 계획의 두 번째 매핑이 무효이면 첫 번째도 기록되지 않는다
- `BA-091-T22`: 뒤늦게 수신한 과거 관측은 더 새로운 관측을 가리지 않는다
- `BA-091-T23`: 이미 만료된 관측은 Live 수집 성공으로 보고되지 않는다
...
진행 상태(`in-progress`, 절마다 test 본문과 대조): 절 스무 개(`T1`~`T20`) 가운데 replay·Live 판정 13개(`T1`·`T2`·`T4`~`T14`)는 main 게이트에서 초록이다. main `59ca9269`(rc.24)의 게이트 run 36237992799에서 이 13개 ID를 단 testcase가 전부 통과했다(artifact에서 셈). 이 13개의 절 → test는 [AGENTS.md CI 표](../../AGENTS.md#ci-검사-등록)의 `api-quality` Live 조회·후보·replay 행에 있고(`T15`~`T18`의 test는 `apps/web` suite·E2E 행에 있다), report는 `apps/api/build/test-results/{test,integrationTest}/TEST-io.nullnull.live.*.xml`과 `TEST-io.nullnull.operations.application.DemoCapabilityQueryTest.xml`(`T11`)이다.
...
**`T3`은 원래 절 여섯을 한 ID에 묶고 있었다**(*"전체 P0 익명 외부망·KO/EN·keyboard·출처·위치 OFF·rollback gate가 통과한다"*). 규칙 3에 따라 `T3`·`T15`~`T19`로 나눴고, 위치 OFF는 지도 SDK 상태에 따라 다시 `T18`·`T20`으로 나눴다. **FE가 #394에서 `T15`~`T18`을 게이트 test 제목에 달았다.** `T15`·`T16`·`T18`은 E2E JUnit으로 #396 게이트에서 처음 집계됐고, `T17`(vitest)은 #393의 vitest 집계가 main에 들어간 뒤 main `59ca9269`의 게이트에서 집계됐다. 넷 다 절을 **일부만** 잰다(아래). `T3`·`T19`·`T20`은 그 ID를 단 testcase가 없어 카드는 `integration-ready`로 올리지 않는다. 넷이 공통으로 재지 않는 것은 절이 말하는 **최종 release**다. `T15`·`T16`·`T18`은 지도 키 없이 만든 게이트 bundle을 로컬 API나 fixture에 붙여 재고, `T17`은 소스를 정적으로 scan할 뿐 bundle을 재지 않는다. release bundle(`staging-release.yml`이 키를 넣어 따로 build한다)이나 그 release의 실제 API에 닿는 것은 없다. 게이트가 재는 부분과 남은 것:

- `T3`(익명 외부망): 재는 것은 사람의 완주뿐이다. staging flows는 verifier 경로라 해당하지 않는다. 최종 release에서 새 시크릿 창·휴대전화 데이터망으로 한다. **Wave 4(rc.24)에서도 채워지지 않았다.** 그때 오너의 외부망 완주는 여행 생성 → 장소 → 최적화 제안 → APPLY/KEEP이었고, Live 등 나머지 P0 흐름을 완주한 기록은 없다. 제출 release가 R5로 바뀌므로 R5 재수집 뒤 다시 판정한다.
...
- `T20`(위치 OFF, Kakao 지도 SDK 켜짐): 게이트에서 잴 수 없다. 게이트 build에는 `VITE_KAKAO_MAP_APP_KEY`가 없고 egress가 막혀 SDK가 로드되지 않는다. 지도 키가 들어간 staging에서 사람이나 operator가 확인한다. **Wave 4(rc.24)에서도 채워지지 않았다.** 수집 기록에 Live 지도 화면을 연 확인이 없다. 오너 완주의 '위치 prompt 없음'은 그 흐름(여행·장소·최적화)의 화면에 대한 관찰이다. release workflow(`staging-release.yml`)는 저장소 변수 `VITE_KAKAO_MAP_APP_KEY`를 bundle build에 넘기고, 그 이름의 변수가 등록돼 있다(`gh variable list`로 이름만 확인했고 값은 보지 않았다). rc.24 bundle이 SDK를 싣는지는 이 카드에서 재지 않았다. R5 재수집 뒤 다시 판정한다.
- `T19`(rollback): Wave 4 안에서는 잴 수 없다. rollback이 edge를 닫고 `current.json`을 바꿔 같은 release의 증거를 모두 무효로 만든다. `BA-071-T4`와 같은 자리다. [#305](https://github.com/yutakdv/Nullnull/issues/305) 본문의 오너 결정은 rollback 연습을 하지 않고 `BA-071-T4`를 미증명으로 남기는 것이다. Wave 4도 rollback을 하지 않았고 `T19`는 채워지지 않았다. `T19`를 같은 방식으로 미증명으로 닫을지는 따로 정한다.

`T15`~`T18`의 ID는 FE가 #394에서 test 제목에 달았다. 이 카드는 제목을 고치지 않았다. 위 판정은 그 test 본문을 읽어 대조한 것이고 test를 다시 돌리지 않았다. 게이트 통과는 #396의 docker-integration artifact에서 셌다(`T15` 6건, `T16` 5건, `T18` 24건, 실패·skip 0). main `59ca9269`(rc.24)의 게이트 run 36237992799 artifact에서도 셌다: `T15` 6건, `T16` 5건, `T17` 3건(vitest), `T18` 24건, 실패·skip 0. `T15`·`T16`·`T18`은 게이트 bundle을, `T17`은 소스 정적 scan을 잰 것이고, 어느 것도 최종 release bundle이 아니다. 원래 `T3`을 인용한 기록(`DECISIONS_AND_RISKS` A-054·A-033)은 나누기 전의 여섯 절 전체를 가리킨다.
