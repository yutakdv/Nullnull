...
- 부분(T7): staging API 로그 그룹에 다섯 이름의 `ops.alarm name=<NAME> drill=BA-072-T7` 줄을 넣었고(관리자 profile, 거절 0건), 배포된 metric filter 다섯이 그 문구를 그대로 인용하며 `OpsAlarmDeletionFailed` metric이 그 분에 1을 기록했다. **metric filter까지는 증명됐다.** alarm의 ALARM 전환 이력과 primary 수신자의 메일 도달은 확인하지 못했다 — 다섯 alarm이 그 뒤 평가 구간에서 `OK`였고 메일이 보이지 않았다. A-043에 따라 secondary 수신자는 두지 않으므로 secondary 절은 범위 밖이다.
