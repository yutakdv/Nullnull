...
| A-054 | Live(`BA-090`·`BA-091`·`BA-092`)를 **제출 전에 전면 구현**한다. `A-033`의 *"구현하지 않고 목업으로 처리할 수 있다"* 와 [#64](https://github.com/yutakdv/Nullnull/issues/64)의 2026-09-19 오너 코멘트 *"제출(09-21) 뒤로 미룹니다"* 를 대체한다 | 2026-09-20 오너 결정. **두 세션에서 각각 오너에게 직접 확인했다** — 어제 코멘트와 정반대라 릴레이만으로 진행하지 않았고, 작업자가 세 선택지(example만·전면 착수·중단)를 마감과 함께 다시 제시해 같은 답을 받았다. **이 순서를 가능하게 한 사실은 같은 날 서울 실시간 도시데이터 실 키가 발급된 것**이다 — `SOURCE_CATALOG` §5의 *"sample key는 제한된 장소만 조회할 수 있다"* 제약이 해당 없어졌다. 남은 것은 셋 다 우리 쪽 배선이다: `.env.example`·compose의 키 자리(값은 비우고 env에서만 읽는다), `SEOUL_CITYDATA`·`DEMO_REPLAY`의 `approval_state` `DISABLED` 승격, 그리고 그 둘이 선 뒤에야 켤 수 있는 `FEATURE_LIVE_DATA`(source registry·key·readiness 없이 ON이면 startup이 실패하는 fail-closed가 이미 걸려 있다). **`A-033`의 `BA-092-T3` 절은 뒤집힌 것이 아니라 이제 원래 의미로 발동한다** — 목업이 아니므로 그 절이 검사하는 것은 Live를 포함한 전체 P0 게이트 그대로다. **마감은 2026-09-21 16:00이고 `BA-090`의 선행 `BA-073`(제출 스트림)이 아직 `planned`다** — 구현은 진행하되 승격과 배포 순서는 제출 증거를 깨지 않는 자리에 둔다(#305의 4~6은 재배포하면 다시 한다). **2026-09-21 실행됐다** — `infra/src/staging.ts` 가 `FEATURE_LIVE_DATA: "true"` 를 API task 에 넘기고, `infra/test/staging.test.ts` 의 단언이 *안 넘어감* 에서 **API 는 ON · ai/ops/migration 은 미설정 · `FEATURE_REPLAY_MODE` 는 계속 금지**로 바뀌었다(infra `npm run check` **37/37**). **그 test 가 이 결정과 배선이 어긋나지 않게 잡는 장치다** — 실행 전에는 그 파일이 정반대를 단언하고 있었고, 그것이 달고 있던 주석(*"live and replay have no source yet"*)은 `V046` 승격과 `WITHOUT_A_SOURCE` 에서 `live` 가 빠진 시점에 **이미 거짓이었다**. 같은 커밋에서 `ENVIRONMENT.md` 의 낡은 두 곳(표 `:242`·산문 `:257`)과 `DemoCapabilityQuery` class javadoc 도 고쳤다 — 셋 다 *"live 는 source 가 없어 켜면 startup 이 실패한다"* 를 적고 있었다 |
| A-063 | **경로 응답에서 유도된 비교 scalar 는 `A-055` 밖이다 — 비교값은 저장하고 피연산자는 저장하지 않는다.** `optimization_proposals.travel_minutes_delta` 는 저장해도 되고, `baseline_travel_minutes`·구간별 duration·distance·geometry 와 **그 조합으로 피연산자가 복원되는 것**은 안 된다 | 2026-09-21 오너 결정. **근거 셋**: (1) `A-055` 가 지키는 것은 제공자의 데이터 산물이다 — 인용된 답변이 허용하는 것은 장소ID·URL, 금지하는 것은 카카오가 **발행하는** 값(duration·distance·geometry)이고, 두 일정 사이의 delta 는 **우리 일정에 대한 진술**이지 카카오가 발행한 적 없는 수다. (2) **되돌릴 수 없다** — delta 하나로 어느 피연산자도 복원되지 않으므로 위장한 캐시가 아니고, 이 비가역성이 *유도* 와 *저장* 을 실제로 가르는 시험이다. (3) **그리고 그 delta 는 구성상 감사 불가능하다 — 이것이 허가 질문에 답하는 유일한 다리다.** **여기 처음 적었던 논거는 틀렸고 측정으로 무너졌다**: *"crowd 선례가 옮겨지지 않는다"* 고 썼는데 `optimization_proposals` 는 **이미 `crowd_delta` 를 저장한다**(`V029:33`, 문제의 `travel_minutes_delta` `:37` 에서 네 줄 위). 저장소는 proposal 에 비교 scalar 를 저장하는 일을 이미 하고 있으므로 선례는 **옮겨진다.** 그리고 원래 논거는 *"delta 가 유일하게 남는 기록이니 지켜야 한다"* 는 **필요성**에서 나왔는데, **필요성은 허가 질문에 답하지 않는다.** `A-055` 는 허가에 대한 것이다. **옮겨지지 않는 것은 선례가 아니라 뒷받침이다**: `crowd_delta` 옆에는 `before_snapshot_id`·`after_snapshot_id` 가 있고(`V034:20-21`, 둘 다 null 이거나 둘 다 채워진다), `OptimizationProposalReader` 는 비교 point 가 사라진 proposal 을 **내보내지 않고 던진다** — 그 주석이 *"한쪽이 없는 채로 내보내면 응답이 나르지 않는 증거 위에 비교가 선 것처럼 보인다"* 고 적는다. `travel_minutes_delta` 에는 그런 쌍이 없고 **가질 수도 없다**(피연산자 저장이 바로 `A-055` 가 막는 것이다). 그래서 route delta 는 provenance 없이 나가고, **펼쳐 낼 피연산자가 없으며, 어떤 독자도 그것에서 provider 값을 복원할 수 없다.** **되돌릴 수 없는 scalar 는 provider 데이터의 캐시가 아니고**, 받은 데이터를 캐시하고 신선하게 유지하는 것이 운영정책 제5조 제20항이 이름 붙인 바로 그것이다. **그리고 이 다리가 아래 경계 규칙을 *번다*** — 감사 불가능성은 **피연산자 열이 delta 옆에 없는 동안에만** 성립하고, 하나라도 생기는 순간 쌍이 복원 가능해져 이 논거가 증발한다. 즉 그 규칙은 위생이 아니라 **허가가 참인 조건**이다. **확신도**: 추론 위의 추론이다 — `A-055` 자신이 카카오모빌리티 vs Local 추론이라고 라벨을 달고 있고 이 줄은 그 위에 얹힌다. 확정할 유일한 방법(서면 질의)은 `A-055` 가 의도적으로 닫았으므로, 이것은 *카카오가 이렇게 말할 것* 이라는 주장이 아니라 **방어 가능하고 검사 가능한 규칙**이다. **규칙을 `V029` 주석에 두지 않는다** — 적용된 migration 이라 못 고친다(원장 규칙 *"고칠 수 없는 파일에 규칙을 적지 마라"*). **남은 작업**: `optimization_proposals` 의 어느 열이 route 유도값을 담아도 되는지 test 로 고정한다. 그것이 없으면 누가 `baseline_travel_minutes` 를 더하는 순간 delta 열은 그대로인 채 `A-055` 가 깨지고, 이 저장소는 산문 경계가 어떻게 되는지 이미 여러 번 봤다 |
...
| A-060 | 서울 실시간의 **4단계를 제품 5칸 척도의 `1·2·3·4`에 놓고 `5`를 비운다.** `CrowdStage.SOURCES_WITH_REVIEWED_SCALE`에 `SEOUL_CITYDATA`를 그 매핑과 함께 등록한다 | 2026-09-20 오너 결정(*"권장사항으로 진행하고 코멘트에 기록을 남겨 FE가 인지하도록"*). **결정은 오너의 것이고 아래 근거의 저자는 `W-LIVE` 세션이다** — `A-057`·`A-058`과 같은 구분이다. **근거**: 매뉴얼 v8.5가 네 단계를 그 구역 **자기 과거 평균 대비 퍼센트**로 정의한다(여유 ≤50%, 보통 50–75%, 약간 붐빔 75–100%, 붐빔 >100%). **서울의 최상단 "붐빔"은 상한이 없으므로 "우리 척도의 최대"라고 말할 근거가 없다** — 비워 두면 상한 있는 source가 오거나 서울이 세분화할 때 자리가 있다. **4를 5에 넣는 방법은 source에서 유도되지 않는다**(1·2·3·4 / 2·3·4·5 / 1·2·4·5 셋 다 성립한다). 그래서 이것은 제품 판단이고, 작업자가 고르면 그것은 *"검토된 매핑"*이 아니라 **지어낸 임계값**이며 불변식 8과 `CrowdStage` 가드가 정확히 그것을 막는다. **이 결정이 여는 것**: 그 등록 전에는 읽기 경계(`JdbcCrowdForecastQuery`)가 등록되지 않은 source의 저장된 단계를 **null + `SCHEMA_DRIFT`로 바꿔서** 내보내므로(`BA-023-T23`), Live 목록에 혼잡도 값이 없었다. **이 결정이 정하지 않는 것**: 5칸 막대가 4칸만 차는 것을 화면이 어떻게 보이는가 — `FCR-035`(5칸 척도)의 소유자가 FE이므로 `#310`으로 올린다. **남는 것**: 구역 간 비교는 여전히 불가다(각 단계가 그 구역 자기 기준이다). `value`는 NULL로 남고 `ordinalLevel`만 채워진다 |
| A-061 | **공개 API edge 를 지금 연다 — `edge --state open` 이 아니라 `TrafficEnabled` parameter 를 직접 뒤집어서.** 제출 전 마지막 배포 뒤에는 `edge --state open` 을 제대로 돌린다 | 2026-09-20 오너 결정(*"일단 열어도 된다는거지? 너가 기록해두던지 해놔"*). **목적은 오너의 모바일 확인**이다. **이것은 drift 가 아니다** — `TrafficEnabled` 는 `infra/src/staging.ts:857` 의 `CfnParameter`(`allowedValues: ["false","true"]`)이고 뒤집으라고 만든 것이다. 그리고 도구가 읽는 정본이 파일이 아니라 스택이라(`edge_traffic_enabled()` 가 `describe-stacks` 로 읽는다) 직접 뒤집어도 operator 가 그 상태를 그대로 본다. **포기하는 것**: `edge --state open` 이 열기 전에 돌리는 `staging-smoke.sh` 와 `staging-flows.mjs`(session·cookie/CSRF/Origin·trips·catalog 상태를 실제로 통과시키고 자기 owner 를 지운다), 배포 lock, plan 해시 재검증, 전파 폴링. **그 pre-check 이 잡을 문제는 폰에서 바로 보이고 되돌리는 것이 parameter 하나**라는 것이 이 선택의 근거다. **계속 참인 제약 셋**: (1) `A-039` — 열려 있는 동안 DB snapshot 복원을 하지 않는다, 복원이 필요하면 먼저 닫는다. (2) `D-003` — KTO 쿼터는 공유라 공개 트래픽이 개발계정 한도를 먹는다(URL 이 아직 어디에도 안 붙어 있어 지금은 낮다). (3) **배포·롤백이 매번 `TrafficEnabled=false` 로 되돌린다**(`execute` 가 그렇게 한다) — 그래서 **제출 전 마지막 배포 뒤에 반드시 다시 열어야 하고**, 그때는 *"열어도 되는가"* 가 실제 질문이므로 pre-check 이 있는 `edge --state open` 을 쓴다. **여는 것은 `/api/*` 뿐이다** — `/`·`/login`·`/covers/*` 는 이미 공개였다(실측: 200, `/api/v1/health/live` 만 503 `application/problem+json`) |
| A-062 | **`BA-083` 의 DAY/TRIP bounded search 는 `apps/ai` 에 새 endpoint 로 간다.** `ADR-0006` 계산 행의 문자 그대로이고 **둘째 예외를 만들지 않는다** | 2026-09-20 오너 결정. **결정은 오너의 것이고 제시된 선택지의 문안은 `W-ROUTE` 세션과 조율자가 썼다** (`A-057`·`A-058` 과 같은 구분). **모호함의 출처**: 같은 ADR 안에서 계산 행은 `apps/api = 없음(fallback만)` 으로 **앱**을 주어로 하는데 금지 문장은 *Spring `recommendation` package* 로 **package** 를 주어로 한다. DAY/TRIP 은 그 표가 쓰일 때 없던 기능이라 계산 열거가 답을 주지 않는다. **무너진 근거 하나를 적는다** — `W-ROUTE` 가 A 의 대가로 `ADR-0006:310`(*제출 빌드 전에 `apps/ai` 배포 경로가 없으면 fallback-only*)을 들었는데 **그 조건절의 전제가 거짓이다**: `infra/src/staging.ts:991` 의 `AiService` 가 `desiredCount: 1` 로 돌고 cloudMap 이름이 `ai` 이며 circuit breaker 까지 있다(`#280` 으로 생겼다). **조율자가 처음 그 반대로 읽었다** — 검색어가 `apps/ai` 였고 infra 는 `AiTask`·`aiService`·`nullnull-stg-ai` 로 쓴다. 범위가 아니라 **검색어**가 좁았고, 그 상태로 판정했으면 Spring 쪽을 *결정적* 이라고 보냈을 것이다. **귀결 넷**: 내부 계약 operation **5 → 6**, `AGENTS.md` CI 표의 *내부 계약 parity(5 operation)* 행이 같이 움직인다, `recommendationTest` parity 를 양쪽에서 맞춘다, `apps/ai/**` 가 `W-I18N` 작업 영역이라 파일 조율이 필요하다. **따라오는 정리**: `ADR-0006:310` 이 *열린 결정으로 추적한다* 로 끝나는데 배포 경로가 생겼으므로 **그 추적이 낡았다** |
...
| --- | --- | --- | --- | --- | --- |
| D-003 | 개발 계정 쿼터의 단위(인증키별인가 활용신청별인가)와 KTO 이미지 재배포 조건은 무엇인가? production key는 **신청하지 않기로 확정**(2026-09-13 오너, PM-023)했고 제출은 개발 계정으로 간다 | BE/AI | 법정동코드 등 새 operation을 같은 키에 추가하기 전 | 쿼터 guard를 미리 조이지 않는다 — per-API가 맞을 때 용량 절반을 버린다. 이미지는 재배포하지 않고 post 표지는 1st-party만 쓴다(A-024) | 포털 마이페이지 활용신청 상세가 API별 트래픽을 따로 보이는지 확인. 실호출 증거는 확보됨(KTO smoke, `api_ingest_logs`·`collector_runs` COMPLETED). **2026-09-20 절반 답이 왔다**: 오너가 `EngService`(data.go.kr 15101753, 개발계정) 활용신청을 제출·승인받았는데 **키는 기존 `KTO_SERVICE_KEY` 에 통합된다** — 즉 **인증키는 계정 단위**이고 활용신청마다 새 키가 나오지 않는다(오너 확인, 슬롯 길이 64 무변화로 재확인). **그래서 남은 절반이 더 중요해졌다**: 쿼터가 **키 단위로 합산**되면 영문 호출이 **P0 흐름을 떠받치는 국문 쿼터를 먹는다.** 활용신청 단위로 따로 세면 무관하다. **확인 전에는 영문 수집을 상시로 돌리지 않는다** — 확인 방법은 위와 같다(마이페이지 활용신청 상세의 API별 트래픽). 부수로 확인된 것: 코드가 `/B551011/KorService2` 를 **상수로 고정**하므로(`KtoKorServiceProperties.DETAIL_OFFICIAL_PATH`) 영문은 같은 키에 **다른 경로**가 필요하고, `ProviderHostPolicy` 의 host 목록도 같이 본다. **남은 절반에 대한 증거가 저장소 안에 있다** (`HANDOFF-PROMPT.md:797` — 그 세션의 측정을 인용하는 것이고 내가 다시 재지 않았다): 같은 포털의 **15077871 이 개발계정 10,000 을 적는데 15101578 은 1,000** 이다. 한도가 계정·키 단위라면 한 계정이 동시에 1,000 이면서 10,000 일 수 없으므로 **per-API 읽기를 강하게 뒷받침한다.** 확정은 여전히 마이페이지이고 *영문 수집을 상시로 돌리지 않는다* 도 그대로다 — **바뀐 것은 대기의 근거이지 대기 자체가 아니다.** 이 줄이 여기 있는 이유는 원장 밖에만 있으면 다음 사람이 같은 조사를 다시 하기 때문이다. **2026-09-21 오너가 마이페이지를 직접 열어 확인했고 이 절반은 닫힌다 — 쿼터는 API 단위이며, 실제로는 그보다 잘게 센다.** **관측(오너 `[실행함]`)**: `한국관광공사_영문 관광정보서비스_GW` 의 활용신청 상세에서 **`행사정보조회`·`숙박정보조회` 등 항목마다 일일 트래픽이 따로** 있다. **읽기**: 한 활용신청 **안에서도** operation 별로 따로 세므로, 서로 다른 활용신청인 `KorService2` 와 `EngService2` 가 한 통에 합산될 수 없다. 이 관측은 위 15077871(10,000) 대 15101578(1,000) 방증과 같은 방향이고, **그 방증과 달리 이번엔 기구로 직접 읽었다** — 이 행이 *"확정은 여전히 마이페이지"* 라고 지정해 둔 바로 그 기구다. **등급 구분**: *항목마다 트래픽이 따로 보인다* 는 관측이고, *따라서 한도도 항목마다 걸린다* 는 읽기다. 후자를 뒤집을 값은 아직 아무도 보지 않았다(항목별 한도 수치 자체). **그래서 풀리는 것**: `영문 수집을 상시로 돌리지 않는다` 제약. 영문 호출은 국문 operation 의 일일 트래픽을 먹지 않는다. **열리는 것**: `BA-086`(#60)의 영문 경로 — `KtoEngServiceProbeMain` 1회 실호출(오너 승인) → 실제 응답 shape 관측 → `KTO_ENG_SERVICE` 를 **실제** license·attribution·schema version 으로 등록 → 제한 표본 수집 → coverage·KO/EN parity 측정 순서로 `T1`~`T3` 이 열린다. **추측한 registry 값으로 상태를 올리지 않는다.** **여전히 열려 있는 절반**: KTO 이미지 재배포 조건(이 행의 두 번째 질문)은 답이 없고 `A-024` 의 provider 사진 금지가 그대로다 |
...
| R-040 | **제출 배포본의 큐레이션 표지 5장이 `A-024`가 금지한 "실제 장소를 사진처럼 묘사한 합성 이미지"에 해당한다.** 다섯 파일 전부 C2PA 매니페스트에 `gpt-image`·`trainedAlgorithmicMedia`·`OpenAI Media Service API`·`c2pa.created`를 담고 있고 `ingredient`/`c2pa.opened`(촬영본 편집의 증거)는 **0건**이다. 배포본에도 같은 바이트가 나간다 | M | H | **2026-09-20 오너가 교체하지 않기로 했다**(마감 15시간, 5장 재촬영·재제작 비용). **`A-024`는 개정하지 않았다** — 금지 조항은 그대로이고 이 다섯 장이 그 예외로 남는다. 완화로 한 것은 **기록을 참으로 만든 것뿐**이다: `covers/README.md`와 `infra/src/staging.ts`가 *"오너가 직접 촬영"* 이라고 적던 것을 정정했다. **기계 검사는 이것을 막지 못한다** — `test_curated_post_covers.py`는 checksum·URL·파일 목록만 보고 `FirstPartyCoverIT`는 FK 만 강제한다(`A-024` 본문이 스스로 적는 그대로다) | **증거가 산출물 안에 서명돼 들어 있다.** 심사자·제3자가 배포 URL 에서 표지를 받아 Content Credentials 를 읽으면 "AI 생성"이 나오고, 그 시점의 화면 맥락은 실제 장소를 찍은 사진이다. 데이터 진실성 항목의 실패 신호에 해당한다. **완화 수단은 교체뿐이고 그것이 배제됐으므로 이 위험은 제출까지 그대로 남는다.** 재발 방지로 할 수 있는 것: 배포 assembly 에 들어가는 이미지의 C2PA `digitalSourceType` 을 보는 검사를 `staging_operator.py` 쪽에 두는 것(이번 제출 범위 밖) | 오너 | **H** |
| R-041 | **불변식 3의 핵심 절(`AI/optimizer 는 preview 만 만들고 사용자 APPLY 전에는 trip 을 수정하지 않는다`)을 소유한 카드 절이 어느 카드에도 없다.** 측정: `BA-050-T1`~`T8` 여덟 절을 전부 읽었고(요청 shape·run/job transaction·lease·동시 claim·freeze·status 역행·직렬화 key) 그 절이 없다. `BA-052`는 결정 **이후**(APPLY/KEEP 멱등·부분 적용 0)를 덮고 결정 **전**을 안 덮는다. `BA-084-T3`이 그 절을 *model OFF* 와 한 ID 에 묶고 있었는데 `apps/ai` 에는 DB 도 mutation 경로도 없어(ADR-0006) 원리적으로 증명할 수 없으므로 묶음을 풀고 앞쪽을 뗐다 | M | H | **`절이 없다`는 측정이고 `test 가 없다`는 안 쟀다** — 규칙 3의 거울상(증명은 있는데 ID 가 없음)일 수 있다. `BA-050`·`BA-051`·`BA-052`가 전부 `integration-ready` 라 증명할 이름 없는 절을 지금 더하면 승격이 그 자리에서 깨지므로, 제출 뒤에 (a) 기존 test 본문에서 그 절을 재는 것을 찾고 (b) 있으면 `BA-050`에 절을 더해 그 이름을 지목하며 없으면 test 를 쓴다 | `BA-050` 재승격, 또는 DAY/TRIP scope 가 열릴 때(`BA-083` step 3) | BE/AI | M |
| R-042 | **`KTO_KOR_SERVICE_2` 의 `current_revision` 을 올리면 그 뒤 국문 표시명이 `canonical_name` 으로 떨어진다.** `BA-086` 의 provenance 게이트는 locale 을 가리지 않고, `place_localizations` 의 유일한 production writer 가 `JdbcCanonicalCatalogStore` 이며 그 경로가 국문 ingest 다. 가설이 아니다 — 그 source 의 revision 은 이미 **세 번** 올랐다(`V008`→2, `V009`→3, `V012`→4, 셋 다 `WHERE code = 'KTO_KOR_SERVICE_2'`) | M | H | **fail-closed 자체는 되돌리지 않는다** — 대안(국문에 provenance 를 안 쓴다)은 생산자 없는 가드이고 이 저장소가 `place_hours`·`place_relations` 에서 두 번 겪은 모양이다. 대신 **운영 금지로 이름 붙인다: 제출 전에는 그 source 의 revision 을 올리지 않는다.** 올려야 하면 같은 migration 에서 `place_localizations.source_registry_version` 을 함께 올리거나 재수집한다 | `source_registry` 의 `KTO_KOR_SERVICE_2` 행을 건드리는 migration 이 제안될 때 | BE/AI | M |
| R-043 | **`places.canonical_name` 이 provider 산문의 게이트 없는 사본을 들고 있다.** `KtoSnapshotCatalogIngest` 가 `snapshot.title()` 을 `canonical_name` 과 localization **양쪽에** 쓰는데, `canonical_name` 은 locale 사슬의 게이트 없는 꼬리이자 게이트 없는 검색어다(`JdbcCatalogPlaceQuery:159`). 담당 세션이 실제 ingest 모양으로 재현했다: localization 철회 뒤 `address` 는 null 이 되는데 **`name` 은 바이트 동일**이고 그 이름으로 **검색도 된다** | M | M | **이 카드에서 열지 않는다** — 닫으려면 `places` 에 provenance 열(migration)이나 `canonical_name` 에 provider title 을 복사하지 않는 더 큰 의미 변경이 필요하다. **완화 근거 둘**: (1) 이 누수는 텍스트가 `canonical_name` 과 같을 때만 나므로 **국문 행에만** 해당하고 영문 coverage 에는 게이트가 온전하다(`T7`·`T8` 이 그것을 잰다). (2) 출처가 사라지는 것은 아니다 — `place_external_refs` 가 `(source_code, source_registry_version)` 을 여전히 pin 한다. 빠진 것은 **철회가 그 열에 닿지 않는 것**이다. `BA-086-T7`·`T8` 의 절 문구는 여전히 참이고(행은 안 나간다) 거짓이 된 것은 *그러면 그 텍스트가 안 나간다* 는 **함의**다 | `places` 에 provenance 를 들이는 카드가 생길 때, 또는 provider 가 텍스트 철회를 실제로 요구할 때 | BE/AI | M |
| R-044 | **localization 철회는 편도다 — 돌아오는 경로가 없다.** `KtoSnapshotCatalogIngest:43-44` 가 `findByExternalReference(...).orElseGet(create)` 라 external ref 가 이미 있으면 그대로 돌려주고 끝난다. **localization 텍스트도 revision pin 도 갱신하는 경로가 어디에도 없다.** 그래서 *re-ingest 하면 돌아온다* 는 **어떤 코드도 뒷받침하지 않는 문장**이고, 담당 세션이 그것을 `V047` 주석에 적었다가 **머지 전에 걷어냈다**(migration 주석은 적용되면 정정할 수 없다) | M | M | **철회가 fail-closed 인 것 자체는 옳다** — 빠진 것은 결함이 아니라 복구 경로다. 지금은 `R-042` 의 운영 제약(*제출 전에는 `KTO_KOR_SERVICE_2` revision 을 올리지 않는다*)이 이것을 **덮고 있지만 덮는 것과 답이 있는 것은 다르다.** 답을 만들려면 ingest 에 갱신 경로(upsert)가 필요하고 그것은 *언제 provider 텍스트를 덮어쓰는가* 라는 별개 결정을 연다 | 제출 뒤 첫 revision 상승, 또는 `place_localizations` 에 두 번째 writer 가 생길 때 | BE/AI | M |
| R-045 | **catalog 공개 게이트가 readiness 에도 제출 검사기에도 안 보인다.** `nullnull.catalog.public-enabled` 는 기본값이 **`false`** 이고(`application.yaml:124`), 그것이 닫히면 `searchPlaces`·`getPlace`·`listRelatedPlaces`·`listFeed`·`getPost`·`getTrip`(item 있는 경우)·`previewTripDraft`·trip 생성/편집·후보·trip-import 가 **전부 503 `SOURCE_UNAVAILABLE`** 이다(게이트 호출 지점 **6개 모듈 19곳**, 실측). **그런데 `getDemoReadiness` 가 그것을 공개하지 않는다** — `DemoCapabilities.NAMES` 는 `live`·`replay`·`optimization` 셋뿐이고 operations 모듈의 그 flag 참조는 **0건**이다(실측). 그래서 `check_submission_inventory.py` 도 못 잡는다: 그 기능들은 ledger 에서 `capability: null` 이고 **검사기는 `null` 인 기능의 readiness 를 아예 안 본다**. `live`·`optimization` 이 꺼지면 같은 검사기가 **25건**을 잡는데 이 게이트는 **같은 크기의 효과를 내면서 0건**이다 | L | H | **지금은 막혀 있다 — 구조가 아니라 값이 막는다**: `infra/staging.config.json` 이 **tracked 파일**이고 `{"catalogPublicEnabled": true}` 다(실측). `infra/src/staging.ts:46-49` 가 그 파일을 읽어 정확히 그 키 하나만 받고 아니면 throw 하며, `:1068` 이 task 환경변수로 넘긴다. **그래서 *켜는 걸 잊는* 실패 모드는 없다** — 손으로 넣는 env 가 아니다. **남는 위험은 그 파일이 바뀌는 것**이고 그때 잡는 것이 아무것도 없다. **검증은 curl 한 줄이다**: 제출 release 에서 `GET /api/v1/places?...` 또는 아무 catalog 읽기가 **503 이 아닌 것**을 확인한다 — readiness 로는 볼 수 없다 | `infra/staging.config.json` 이 바뀔 때, 또는 catalog 게이트를 쓰는 module 이 늘어날 때 | BE/AI | L |
