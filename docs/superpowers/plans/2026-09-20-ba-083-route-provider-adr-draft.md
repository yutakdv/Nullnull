...
`radius`가 **필수**이고 최대 10,000m다 — 단 **그것은 이 표의 다중 목적지·다중 출발지 이야기이고 1→1 자동차 길찾기에는 해당하지 않는다.** A-051/1→1 결정 뒤 `GET /v1/directions`를 `radius` 없이 호출해 **HTTP 200 · `result_code: 0`** 을 받았다(2026-09-20, 오너 승인 실호출 1건). 그래서 이 문서가 §11에 *의미 미확인*으로 남겨 둔 `radius` 항목은 **채택된 호출 형태에서는 존재하지 않는다.** **[1차·실측]**
...
- `/notice/21`(도메인·네트워크 안내)은 **404**였다. egress allowlist에 넣을 호스트 `apis-navi.kakaomobility.com`은 **실호출로 확인됐다**(아래). 추가 도메인이나 IP 대역이 있는지는 여전히 확인하지 못했다 — 확인된 것은 *이 호스트가 답한다*이지 *이것뿐이다*가 아니다.
- 다중 출발지의 **초과 단가**가 가격표에 없다. 무료 1,000건/일만 확인했다.
- ~~실제 호출은 한 번도 하지 않았다~~ — **2026-09-20에 오너 승인으로 1건 돌렸다**(`GET /v1/directions`, 공개 랜드마크 좌표 둘, HTTP 200). 닫힌 것과 안 닫힌 것을 나눠 적는다.
  - **닫힘**: 응답이 `routes[0].summary.{distance, duration}`을 주고 `duration`은 **초**다(실측 `distance=6308`m, `duration=1114`s — 6.3km 서울 도심 18.6분으로 산수가 맞는다). `result_code`·`result_msg`는 **`routes[0]`에** 있고 성공이 `0`/`길찾기 성공`이다. `routes`는 1→1에서도 배열이다.
  - **닫힘**: `radius`는 1→1에 불필요하다(§3 각주).
  - **안 닫힘**: **`result_code`가 0이 아닌 값들의 어휘는 여전히 미확인이다.** 호출이 1건이었고 그것이 성공했다. 그래서 gateway는 `0`만 `Available`로 만들고 나머지는 전부 `Absent`로 두며, **`Leg.Unavailable`은 생산자가 없는 상태로 등록한다**(`BA-083-T22`·`T23`) — 모르는 실패를 *"provider가 경로가 없다고 답했다"* 로 부르는 것은 지어내는 것이다.
  - **응답 본문은 보관하지 않았다.** `A-055`의 공식 답변이 *"개발용 샘플 응답 저장도 불가"* 라고 명시하므로 구조(필드 이름)만 뽑고 body는 지웠다. fixture로 만들지 않았다.
  - **`summary=true`로 응답을 줄일 수 있는지는 미확인이다**(문서에 있다고 읽었을 뿐 호출로 확인하지 않았다). 켜는 것은 gateway가 끈 상태로 초록을 본 뒤에 시도한다.
