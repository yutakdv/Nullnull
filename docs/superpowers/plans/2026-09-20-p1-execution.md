...
| BA-086 영문 coverage | 없음 | `V047`·`V048` | 노출 필드 생기면 | FE 의존 가장 적음 |
| BA-080 검색·filter | BA-085 뒤 | `V042` | 신규 | `/search` 디자인 없음 |
| BA-083 경로·DAY/TRIP | ADR 먼저 | 없음 | 신규 | 카카오 ADR → gateway → matrix |
...
**이 문단의 원래 예약표는 낡았다** — `V037`~`V046` 은 전부 소진됐고(실측: tracked 최상단 `V046`) 예약된 용도와
다르게 쓰였다. 현재 배정(조율자가 정하고 방송한다): `V047`·`V048` BA-086 영문 · `V049` BA-092 replay manifest.
**BA-083 경로는 migration 0건이다** — `A-055` 로 경로 응답을 저장하지 않으므로 route snapshot table 을 만들지
않는다(담당 세션이 재서 확인했다). 예약 번호를 보고 쓰지 말고 **커밋 직전에 최상단을 다시 재라.**
