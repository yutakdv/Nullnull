...
- **`withdraw-post`는 표지 이미지를 지우지 않는다.** 성공할 때마다 `post_withdraw_residual=cover-object-not-deleted`를 찍는 이유가 이것이다. 사용자가 올린 표지(`covers/user/…`)는 원래 URL에서 계속 서빙된다. 세 가지가 겹친다: API·ops task role과 operator role 어디에도 그 prefix의 객체 삭제 권한이 없다. web bucket이 versioned라서 지워도 이전 버전이 남는다. 객체가 `Cache-Control: public, max-age=31536000, immutable`로 나갔으므로 이미 받아 간 브라우저 캐시는 회수할 수 없다. 권리 철회처럼 이미지 자체가 사라져야 하는 경우는 이 명령만으로 끝나지 않는다. 게시물 id도 완전히 사라지지 않는다. 회수 전에 저장한 여행 후보는 출처로 그 id를 계속 들고 있다. 새 POST 출처 후보 저장은 같은 transaction에서 PUBLISHED를 잠그고 확인하므로 회수와 직렬화된다. 회수 commit 뒤의 새 저장은 DRAFT·없는 id와 같은 404이며, 이미 commit된 idempotency key의 재시도는 새 출처를 만들지 않고 replay한다. 나가는 것은 기존 id뿐이고 게시물 내용은 나가지 않는다.
- **#338 신규 표지 캐시 정책:** 이 수정 배포 후 새로 발행하는 `covers/user/…` 객체는 `Cache-Control: no-store`다. CloudFront 기본 S3 behavior의 `CACHING_DISABLED`는 유지한다. 기존 객체의 metadata·이전 버전·이미 받아 간 사본은 바꾸지 않는다. service worker의 Cache Storage는 HTTP 캐시와 별개이므로 헤더만으로 삭제되거나 저장이 막혔다고 판단하지 않는다.
- **권리 철회 사건의 완료 기준을 분리한다:** (1) 승인한 post id로 `withdraw-post`를 실행하고 요청 id와 일치하는 성공 줄을 보관한다. (2) 새 요청의 getPost 404·feed 제외·새 POST 출처 후보 404를 확인한다. (3) 표지 URL의 origin 객체와 모든 이전 버전 삭제, CDN 잔여 확인은 별도 최소권한·검토된 삭제 경로가 필요하며 현재 도구는 수행하지 않는다. `cover-object-not-deleted`가 남아 있는 사건을 이미지 회수 완료로 기록하지 않는다. (4) 브라우저·service worker·다운로드·제3자 복제 사본은 서버에서 강제로 회수할 수 없음을 사건 기록에 남긴다. 이 runbook 변경은 삭제나 IAM 변경을 실행한 증거가 아니다.
