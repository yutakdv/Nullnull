...
- [ ] 미완성 P1은 capability OFF이며 JA/ZH·알림·주변·DAY/TRIP 최적화 control이 disabled/준비 중이다. 로그인은 `A-075`대로 /sign-in 흉내(공모전 테스트 계정 prefill, 브라우저 안 대조, 요청 0건)와 프로필 `TEST` 표기이고, 로그인 없이 쓰는 경로는 그대로다. **게시물 작성은 2026-09-20부터 제출 범위이므로 이 목록에 넣지 않는다**(`A-058`).
