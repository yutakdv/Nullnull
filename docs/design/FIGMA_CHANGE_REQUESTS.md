...
| FCR-018 | P0 blocker | 438:3158 untimed 필수 장소→384:5673 초안, 수동 시작/확인 단계 불일치 | 확인 전 초안 계약·세 분기·수정/만료·확정 경계; PM-004 | FE / BE·AI·PM | Open · 확인 전 초안 계약은 저장 없는 `previewTripDraft`로 확정됐다([#247](https://github.com/yutakdv/Nullnull/issues/247) → [#248](https://github.com/yutakdv/Nullnull/pull/248), 2026-09-18). wizard는 preview를 먼저 보여 주고 `Pick`을 켠 stop만 `MUST_VISIT` lock으로 `createTrip`의 `seedItems`에 옮긴다(`FE-102-T4`, `wizard-screen.test.tsx`·`trip-create.spec.ts`). EMPTY는 빈 일정 성공으로 그리지 않는다(`FE-102-T5`). 입력 방법 선택 `400:1201`은 wizard step 4 `InputMethodStep`으로 구현됐다(`FE-103`). 남은 것은 종료 조건 1(Figma 정정과 전후 screenshot — 이번에 Figma를 열어 보지 않았다)과 4(PM-004 승인)다 |
