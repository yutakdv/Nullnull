...
| FCR-006 | P0 blocker | S14 `422:2925`가 `로그인하면 일정을 저장할 수 있어요`와 활성 login affordance를 노출 | `이 기기의 익명 세션에 저장돼요`처럼 실제 보존 방식을 설명하고 login은 disabled `준비 중`; 요청 0건 | FE / BE·AI·PM | A-075(오너 2026-09-25)가 대체: /sign-in의 공모전 테스트 계정 prefill(브라우저 안 대조, 로그인·인증 요청 0건)과 프로필 `TEST` 표기를 유지한다. 이 행의 `준비 중` 요구는 적용하지 않는다. 이전 기록: Figma 수정 완료 · 검토 대기 (2026-09-07, [증거](#fcr-006-증거)) |
...
| FCR-015 | P0 blocker | 적용 완료 `417:2412`의 되돌리기가 toast action뿐이고 적용 대상 revision·24시간 `revertUntil`·만료 상태를 지속적으로 확인할 수 없음 | `ApplyOptimizationDecision`의 전후 revision·`revertUntil`을 persistent UI로 표시하고 가능/진행/완료/`REVERT_WINDOW_EXPIRED` 상태를 제공. toast는 보조 피드백으로만 사용 | FE / BE·AI | 퇴역(A-074, 오너 2026-09-25): 여행 화면의 되돌리기 진입점이 없다. 이전 기록: Figma 수정 완료 · 검토 대기 (2026-09-08, [증거](#fcr-015-증거)) |
...
| FCR-017 | P0 blocker / FE P1 time edit | 527:4085는 날짜 이동 결과, 527:4695는 후보 신규 일정화 날짜 선택 | P0는 node 의미 정정·선택 Day 일치·삽입 위치 정책을 확정한다. 별도 시간/duration 편집은 FE 제출 범위에서 P1로 이관하고 공유 우선순위는 BA-040과 함께 갱신한다; PM-003/008 | FE / BE·AI·PM | Open |
| FCR-018 | P0 blocker | 438:3158 untimed 필수 장소→384:5673 초안, 수동 시작/확인 단계 불일치 | 확인 전 초안 계약·세 분기·수정/만료·확정 경계; PM-004 | FE / BE·AI·PM | Open · **이 행의 흐름(필수 장소 `438:3158` → 초안 `384:5673`)은 구현되지 않았다.** MUST_VISIT_ONLY 분기의 must-visit step은 `이대로 채우기`에서 preview 없이 `createTrip`을 바로 부르고(`TripWizardScreen.tsx`의 `MustVisitStep` `onSubmit` → `submit(draft)`), `toCreateRequest`(`wizard.ts`)가 `draft.mustVisit`을 보내지 않으므로 담은 장소는 만들어진 trip에 남지 않는다. 담은 곳을 `addTripCandidate`(`mustVisit: true`)로 보내는 N번의 요청은 `createTrip`과 한 transaction이 아니고 일부만 성공했을 때의 화면이 정해지지 않아 배선하지 않았다(`wizard.ts` 주석, [#185](https://github.com/yutakdv/Nullnull/issues/185)의 2026-09-13 FE 질문). 초안 `384:5673`(`RecommendedDraftStep`)은 NOTHING 분기에서만 열리고, 그 `previewTripDraft` 요청에는 필수 장소 입력이 없다(`FE-102-T4`가 요청 key를 `endDate`·`startDate`·`timezone` 셋으로 고정한다). [#180](https://github.com/yutakdv/Nullnull/issues/180)의 2026-09-19 FE 코멘트는 날짜 없는 필수 장소가 오너 판단으로 P0 제출 범위 밖이라고 전하지만 `FUNCTIONAL_INVENTORY.md`의 `FR-TRC-04`는 아직 P0다. 된 것: 확인 전 초안 계약은 저장 없는 `previewTripDraft`로 확정됐다([#247](https://github.com/yutakdv/Nullnull/issues/247) → [#248](https://github.com/yutakdv/Nullnull/pull/248), 2026-09-18). NOTHING 분기는 preview를 먼저 보여 주고, 확정하면 preview의 stop 전부를 `createTrip`의 `seedItems`로 옮기되 `MUST_VISIT` lock은 `Pick`을 켠 stop에만 단다(`FE-102-T4`, `wizard-screen.test.tsx`·`trip-create.spec.ts`). EMPTY는 빈 일정 성공으로 그리지 않는다(`FE-102-T5`). MOSTLY_PLANNED의 입력 방법 선택 `400:1201`은 wizard step 4 `InputMethodStep`으로 구현됐다(`FE-103`). 종료 조건 1(Figma 정정과 전후 screenshot — 이번에 Figma를 열어 보지 않았다)과 4(PM-004 승인)도 채워지지 않았다 |
| FCR-019 | P0 blocker | 401:1221 원문·438:3259 confirm에서 연도/제목/제거 수정 부족 | 날짜 기준·기본 제목·item 제거/remap·전송 안내·IMPORT 잠금; PM-005. BA-060 착수 검토에서 막다른 길의 범위가 **item이 아니라 unresolved token**임이 드러났다 — READY는 token 0건을 요구하는데 `RemapImportRequest`에 token을 **기각**하는 표현이 없어, suggestions가 비는 줄(자유 메모 등)이 하나라도 있으면 그 draft는 confirm에 영영 도달하지 못한다. 오인식 *item* 은 막다른 길이 아니다(다른 place로 remap하거나 confirm 뒤 `removeTripItem`). **BE 제안([#223](https://github.com/yutakdv/Nullnull/issues/223))**: `updates[]`에 optional `dismissed: boolean`을 더해 token은 물리고 item은 draft에서 뺀다 — 이 표가 지적한 **item 제거 부족도 같이 닫힌다**. READY는 `items` 전부 `place != null && date != null`로 정의한다. additive라 breaking이 아니고 FE 승인만 필요하다. 연도 없는 날짜·모호한 오전/오후는 그대로 임의 확정하지 않고 unresolved token으로 낸다 | FE / BE·AI·PM | Open |
| FCR-020 | P0 blocker | 438:3108 동행인/스타일 chip에 canonical code/weight 없음 | 지원 dictionary·선택 규칙·KOEN label·중복/빈 선택; PM-006 | FE / BE·AI·PM | 구현 완료 · 종료 조건 1·3·4·5 대기 — [종료 조건](#종료-조건)은 다섯 모두를 요구하는데 이 기록에는 1(수정된 Figma node URL과 전후 screenshot)이 없고, 3(KO/EN 360px·keyboard·state 확인)이 연결되지 않았고, 4는 BE/AI 검토(아래 `BA-030-T1`)만 있고 PM-006 승인이 없고, 5(Storybook/Playwright ID)가 없다 — 지금 증거는 Vitest와 BE JUnit이며 그것으로 5를 대신할지는 [#13](https://github.com/yutakdv/Nullnull/issues/13) 결정을 따른다. 2는 `FR-TRC-02`↔`replaceTripInterests`(기능 인벤토리)로 충족한다. 어휘 확정(2026-09-12, [증거](#fcr-020-증거), [#154](https://github.com/yutakdv/Nullnull/issues/154) CLOSED). 서버는 `InterestVocabularyContractTest`(`BA-030-T1`)가 계약 `x-nullnull-interest-codes`와 `InterestVocabulary`를 양방향으로 고정하고, 화면 문구는 `message-key-coverage.test.ts`가 `INTEREST_GROUPS`의 모든 code에 KO/EN label이 있는지 본다 |
...
| FCR-022 | P0 blocker | 412:1912 SCHEDULED 제거 action, 저장 결과는 toast 중심 | **후보 전이·교체 linkage는 확정**(#165 Q1·Q2). 남은 것은 독립 후보 보존 — APPLY→REVERT 복원 범위이고 BA-053 소유; PM-009 | FE / BE·AI·PM | Open (종료 판정만) · 남아 있던 REVERT 복원 범위는 `BA-053-T11`(`OptimizeRevertIT`)이 증명한다: REVERT는 기록된 일정 변경만 되돌리고 후보의 ACTIVE·DISMISSED 상태를 바꾸지 않는다. 여행 화면의 되돌리기 진입점은 A-074로 퇴역해 이 경로는 API에만 남는다. 닫는 것은 PM-009 판정을 기다린다 |
...
| FCR-024 | P0 major | feed/post 작성자·하트 수·반응 상태와 read schema 불일치 | **화면 범위 확정**(#163 — 작성자·하트·반응을 P0 화면에서 제거). `FR-FED-04`는 계측(`IMPRESSION`/`OPEN`)만으로 닫는다 — 두 값은 버튼이 필요 없어 화면 변경 없이 기록된다. `LIKE`/`DISLIKE`는 누를 control이 없으므로 P1. **`HIDE`는 P0 미구현**: 복구 진입점이 저장소 전체에 0건이라([증거](#fcr-024-증거)) 지금 구현하면 숨긴 글이 돌아올 길이 없다; PM-011 | FE / BE·AI·PM | 구현 완료 · 종료 조건 1·3·4·5 대기 — [종료 조건](#종료-조건)은 다섯 모두를 요구하는데 이 기록에는 1의 전후 screenshot이 없고(node와 렌더 전후 확인은 [증거](#fcr-024-증거)에 적혀 있으나 `docs/design/evidence/`에 파일이 없다), 3(KO/EN 360px·keyboard·state 확인)이 연결되지 않았고, 4는 BE/AI 검토([#163](https://github.com/yutakdv/Nullnull/issues/163))만 있고 PM-011 승인이 없고, 5(Storybook/Playwright ID)가 없다 — 지금 증거는 Vitest `feed.test.tsx`이며 그것으로 5를 대신할지는 [#13](https://github.com/yutakdv/Nullnull/issues/13) 결정을 따른다. 2는 `FR-FED-04`↔`recordFeedFeedback`로 충족한다. 화면 범위 확정 · 계약·구현 완료 · Figma 반영 완료 (2026-09-18, [증거](#fcr-024-증거), [#163](https://github.com/yutakdv/Nullnull/issues/163) CLOSED). 단서: 이 행이 `FR-FED-04`를 닫는 수단으로 적은 `IMPRESSION`·`OPEN` 계측은 아직 배선되지 않았다 — `recordFeedFeedback`를 부르는 production 코드가 0건이고 `feed.test.tsx`의 `FE-201 #163` case가 요청 0건을 고정한다. 그것은 이 FCR의 화면 범위가 아니라 FE 코드 작업으로 남는다 |
...
| FCR-027 | P0 blocker | ITEM 대상 선택·취소 copy·refresh/만료/이력/undo 상태 불완전 | read 복구·back≠cancel·실패 복귀≠KEEP·서버 undo 가능성; PM-015 | FE / BE·AI·PM | Open · undo 절은 A-074로 퇴역했다 |
| FCR-028 | P0 major | 417:2567 NO_IMPROVEMENT가 전역 최적성을 주장 | 확인한 후보 범위 문구·UNKNOWN/CHECKING/NONE·유효 CTA; PM-020 | FE / BE·AI·PM | Open · FE 문구 반영: 실행 실패 줄 `run.failure.NO_IMPROVEMENT`를 확인한 후보 범위로 좁혔다(`FE-502-T4`). 남은 것은 417:2567 Figma 정정, PM-020, primary CTA 대조(화면 '다시 계산하기' vs handoff '내 여행으로 돌아가기') |
| FCR-029 | P0 major | 필수 장소 `438:3158` 카드가 혼잡 단계(`4 · 혼잡`)·`공식 혼잡 예측`·출처를 표시하지만 `searchPlaces`가 돌려주는 `PlaceSummary`와 `PlaceDetail` 어디에도 crowd 필드가 없음 | Figma 시안은 유지한다. **`PlaceSummary`에는 crowd를 추가하지 않는다**([#105](https://github.com/yutakdv/Nullnull/issues/105) 합의, 09-09 방향을 대체). 혼잡은 `getPlaceCrowdForecast`(`BA-023`, 한 장소 × 기간)로 읽고, 카드는 서버가 준 point 하나를 그 provenance(`targetAt`·`sourceState`·출처 등)와 함께 그대로 표시한다 — 평균 등 새 수치는 만들지 않는다. 검색결과처럼 카드가 여럿인 화면은 batch `queryPlaceCrowdForecasts`(`POST /places/crowd-forecasts/query`, `placeIds[1..50]`, `items[i]`가 `placeIds[i]`에 답한다)로 읽는다. **카드끼리 정렬·비교하지 않는다**(`comparisonAxis`는 `TEMPORAL`뿐, 불변식 8) | FE / BE·AI | FE 구현 완료 · 종료 조건 1은 해당 없음(시안 유지) · 4의 BE/AI 계약 검토는 [#105](https://github.com/yutakdv/Nullnull/issues/105) 합의 · 5의 구현 test는 Vitest `must-visit.test.tsx`의 case `renders the ordered crowd response without deriving a stage`(검색 결과 순서대로 보낸 batch 요청과 그 응답을 카드에 그대로 그리고 `Level N` 같은 단계를 합성하지 않는다)이고 Storybook/Playwright ID를 요구할지는 #13 결정을 따른다 · 단건은 열린 시트와 viewport 진입 카드에서만 조회 · 검색결과는 batch 응답을 index로 연결 · 최대 point와 그 날짜·provenance를 함께 표시 · PR #300의 request/response fixture를 index로 연결해 deprecated 요청 ID → canonical 명동 응답 parity까지 검증 완료(2026-09-20) · 실제 KTO 호출 증거(BA-021-T3)는 [#305](https://github.com/yutakdv/Nullnull/issues/305)(제출 release 최종 검수)가 마지막 배포 뒤에 모은다 — [#33](https://github.com/yutakdv/Nullnull/issues/33)은 그 절을 #305로 넘기고 2026-09-19 닫혔다 |
...
| FCR-033 | P0 major | 오너 결정으로 item 편집 시트(`411:1837`)에 잠금 종류별 독립 toggle을 넣기로 했으나, 이 시트가 덮는 것은 `MUST_VISIT`·`DATE`·`TIME` 3종뿐이다. `RESERVATION`은 `COMPONENT_CATALOG.md`의 잠금 규칙이 `reservation-locked`를 toggle로 제공하지 못하게 하므로(예약은 켜고 끄는 값이 아니라 날짜·시각을 가진 입력) `FR-CON-04`가 갈 자리가 없다 | 예약 잠금을 설정·수정하는 입력 자리를 시안에 정의한다. `setTripItemConstraint`의 `RESERVATION` body가 `date`·`startTime`을 요구하고 `endTime`은 선택이므로 toggle이 아니라 날짜·시각 입력이 필요하다. 자리가 정해지기 전까지 FE는 예약 잠금을 표시만 하고 설정 UI를 만들지 않는다(불변식 7: 네 잠금은 독립이며 자동 해제하지 않는다) | FE / BE·AI·PM | Open · 디자인 부재 (FE-307 범위 확정 중 발견 2026-09-11) |
| FCR-034 | P0 major | S09-3 applied-panel 네 frame(`417:2412` · `724:4602` · `724:4730` · `724:4858`)은 `result-row` · `revision-line` · `btn/revert` 3행이 전부이고 되돌리기가 **실패한** 상태의 frame·문구가 없다. S09 오류 참조 `417:2567`의 여섯 코드는 frame 자신이 `아래 각 상태는 S09 Preview 위 배너 + 액션으로 표시된다`고 적어 preview 화면을 가리키며 revert 실패도 read 실패도 그 목록에 없다. `FCR-015`가 이 패널에 지정한 상태도 가능·진행·완료·`REVERT_WINDOW_EXPIRED` 넷뿐이라 실패는 그 범위 밖이다 | 되돌리기가 실패한 상태의 badge·문구를 시안에 정의한다. 그때까지 FE는 `FCR-016` 선례대로 계약 근거만으로 선구현하고 Figma 확정 시 교체한다 — 문구 둘 다 `일정은 그대로`를 먼저 말하고(실패한 것이 되돌리기이므로 적용된 일정이 남아 있다), 다시 눌러 되는지로 둘을 가른다(`REVERT_WINDOW_EXPIRED`는 problem policy가 `retry: none`·`recovery: none`이라 재시도 버튼을 disabled로 둔다). 실패 줄에는 색을 쓰지 않는다 — 이 디자인 시스템에 danger token이 없고 형제 화면들이 상태 문구에 `--color-text-secondary`를 쓰므로 그 관례를 따르며, 의미는 문구와 `role="alert"`가 나른다(색만으로 의미를 전달하지 않는다). 시안이 실패 상태의 색을 정하면 그때 맞춘다. read 실패(`run.data` 부재)를 `되돌릴 것 없음`과 같은 화면으로 두는 것은 의도된 것으로 남긴다 — 이 패널은 화면이 아니라 여행 화면의 한 섹션이고 그 화면은 살아 있다 | FE / BE·AI·PM | 퇴역(A-074, 오너 2026-09-25): 이 패널을 그리는 화면이 없다. 이전 기록: Open · 디자인 부재 (FE-505 구현 중 발견 2026-09-19) |
| FCR-035 | P0 major | 혼잡 단계 척도가 정본 넷에서 불일치한다. `CrowdBar` 인스턴스(`438:3178`)는 칸이 **5개**이고 같은 화면에 `5 · 매우 혼잡`(`438:3192`)이 `4 · 혼잡`(`438:3179`)과 나란히 있는데, `COMPONENT_CATALOG.md`의 `CrowdLevel` 행은 `1~4단계`, FCR-004가 확정한 어휘는 4단계까지였다 | 척도는 5단계로 고정한다. `CrowdBar`·KO/EN 화면 문구·component catalog·계약 `ordinalLevel`을 같은 척도로 맞추고 수치에서 단계를 합성하지 않는다 | FE / BE·AI·PM | Closed · `CrowdLevel` 5칸과 `1 · 매우 여유`~`5 · 매우 혼잡` KO/EN 문구, component catalog, 계약 `^[1-5]$`가 일치함 (2026-09-20). 서울 변형은 새 FCR 없이 A-060(오너 2026-09-20)이 정한다: 서울의 네 단계는 5칸 중 1~4칸에 놓이고 5칸째는 비운 칸으로 그리며(`FE-403-T4`), 단계 문구는 서울 어휘 `여유`·`보통`·`약간 붐빔`·`붐빔`이고, 막대 이름은 칸 수가 아니라 발행 단계 수로 `4단계 중 N번째`라고 읽는다 — 앱 안에서는 `서울 혼잡도 4단계 중 N번째`(`messages.ts`의 `live.crowd.seoul.levelLabel`, KO `서울 혼잡도 {steps}단계 중 {level}번째`·EN `Seoul crowd level {level} of {steps}`). `COMPONENT_CATALOG.md`의 `CrowdLevel` 행은 그 규칙을 `서울 '4단계 중 N번째'`로만 적는다. 증거는 `FE-403-T4`·`FE-403-T5`다: 컴포넌트 단위의 칸과 이름은 `data-components.test.tsx`가, 앱 안의 KO·EN 이름(`서울 혼잡도 4단계 중 N번째`·`Seoul crowd level N of 4`)과 비운 5칸째는 게이트의 `live-replay-matrix.spec.ts`가 잰다 |
| FCR-036 | P0 major | 예보 미보유 장소의 사유 문구가 없고 계약은 `unavailableReason` token만 보낸다 | `NO_COVERAGE`는 기간 내 예보 없음, `PLACE_UNAVAILABLE`은 해당 장소 예보 제공 불가로 KO/EN 매핑한다. 모르는 token은 raw 값을 노출하지 않고 일반 unavailable 문구로 되돌린다. 숫자·단계·출처는 만들지 않는다 | FE / BE·AI·PM | Closed · 세 경우의 KO/EN 문구와 unknown-token 회귀 검사를 구현함 (2026-09-20) |
| FCR-037 | P0 major | 여행 보기에 날짜 범위·정보 수정 form(`/trip/:id/settings`, `FR-TRP-05`)으로 가는 진입점이 없다. 93de5679가 일정 편집을 S07-2 schedule editor로 옮긴 뒤 앱 안에서는 URL을 직접 쳐야만 여행 날짜를 바꿀 수 있었고, 그 진입점을 그린 Figma node도 없다 | 여행 보기 hero 제목 줄의 이름 변경 연필 옆에 icon link(`여행 기간·정보 수정` / `Edit trip dates and details`)를 두고, S07-2 편집 mode와 제목 편집 중에는 두지 않는다. 열면 focus가 form 첫 field로, 닫으면 그 link로 돌아온다(`FE-306-T4`~`T6`). 위치와 icon은 Figma가 그릴 때까지의 FE placeholder이고 그리면 그 시안을 따른다 | FE / PM | 구현 · Figma node 대기 (FE-306, 2026-09-26. R4 포함은 오너 결정: 오너 배치 전에 준비되면 넣는다) |
| FCR-038 | P0 major | 장소 검색 결과가 첫 page(최대 20개)에서 끝나 20번째 뒤 장소에 어느 검색창에서도 닿을 수 없었다([#54](https://github.com/yutakdv/Nullnull/issues/54) §4). `searchPlaces`의 cursor로 다음 page를 이어 받는 계속하기 control을 넣었지만, 그 control과 상태(불러오는 중·실패·거절된 cursor)를 그린 Figma node가 없다. 필수 장소 `438:3158`·직접 입력 `438:3199`·장소 추가·라이브 검색·게시글 작성의 다섯 검색 목록이 같은 control을 쓴다 | 결과 목록 바로 아래 전폭 pill 버튼 하나를 둔다: `검색 결과 더 보기` / `Show more results`, 받는 동안 `결과를 더 불러오는 중이에요`(`aria-busy`, disabled 아님), 실패하면 받은 결과를 그대로 두고 `결과를 더 불러오지 못했어요. 받은 결과는 그대로예요.`를 알리며 버튼이 `다시 불러오기`가 된다. `CURSOR_EXPIRED`·`CURSOR_INVALID`이면 버튼이 계약 CTA `처음부터 다시 보기`가 되어 첫 page부터 다시 조회한다. 다음 page가 오면 focus는 새 첫 결과로 간다(`FE-103-T10`, 동작 절 전체는 `FE-103-T5`~`T23`). 위치·모양·문구는 Figma가 그릴 때까지의 FE placeholder이고 그리면 그 시안을 따른다 | FE / PM | 구현 · Figma node 대기 (FE-103, 2026-09-26) |

FCR 번호는 기존 참조를 보존한다. FCR-010~015는 09-06 PR #6 계약 검토에서, FCR-016~028은 09-06 직접 canvas 대조와 API 검토에서, FCR-029는 FE-103, FCR-030은 FE-301, FCR-031은 FE-303, FCR-032는 FE-306, FCR-033은 FE-307 범위 확정 중 계약·컴포넌트 규칙 대조에서, FCR-034는 FE-505 구현 중 Figma frame 실측에서, FCR-037은 FE-306 진입점 복구 중, FCR-038은 FE-103 검색 계속하기 구현 중 추가했다. 등록은 문서 기준선이며 디자인 수정 완료를 뜻하지 않는다.
...
오너의 기존 카카오 지도 복구 요청에 따라 `FR-LIV-01`/`FE-401` 구현은 지도와
목록을 함께 제공한다. 아래 표와 기존 증거의 map OFF 기본은 당시 설계 이력이다.
현재 acceptance는 SDK 실패 시에도 목록·검색·상세를 유지하고, 좌표 없는 권역의
marker를 만들지 않으며, 선택 장소에만 `getPlace`의 실제 좌표를 쓰는 것이다.
실제 JavaScript 키·허용 origin·타일·출처 확인은 운영 복구 완료 전에 수행한다.
Figma 원본의 지도 복구 반영은 별도 확인이며 이 문서 변경으로 완료를 주장하지 않는다.
...
- S14 guest: A-075(오너 2026-09-25)로 대체 — 프로필은 `TEST` / `이 계정은 test계정입니다`로 표기한다. 옛 문구(`로그인 없이 시작했어요` / `여행은 이 기기의 익명 세션에 저장돼요.`)는 기록으로만 남긴다.
- S14 login: A-075(오너 2026-09-25)로 대체 — /sign-in 흉내(공모전 테스트 계정 prefill, 브라우저 안 대조, 로그인·인증 요청 0건)와 프로필 `TEST` 표기. 옛 지시(`로그인 · 준비 중` disabled control 또는 숨김)는 기록으로만 남긴다.
...
작성자·팔로우를 제거했고 `FCR-003`이 정렬 chip을 숨겼기 때문이다.

| 확인 | 결과 |
| --- | --- |
| `FeedPostCard`의 작성자·해시태그·하트 | 0건 |
| `PostScreen`의 같은 값 | 0건 |
...
`HIDE`를 enum에서 빼자는 뜻은 아니다 — `additionalProperties: false`라 제거가 breaking
이고 P1에 쓸 값이다. 서버가 받으면 거절하거나, 생산자가 없다는 사실을 계약에 적는
쪽이 맞다(`preserveDateTime`을 422로 거절한 것과 같은 판단, #203).

| 값 | P0 | 이유 |
| --- | --- | --- |
| `IMPRESSION` `OPEN` | 예 | 버튼이 필요 없다. 카드가 보이고 열리는 것이 곧 신호라 화면 변경 없이 기록된다 |
| `LIKE` `DISLIKE` | 아니오 | 누를 control이 화면에 없다. 도달할 수 없는 명령을 구현하면 서버에 영영 오지 않는 코드가 된다 |
| `HIDE` | 아니오 | 복구 진입점 0건 |
| 하트 수 | 아니오 | 집계 소스가 없고 표시할 자리도 없다 |

`FR-FED-04`(P0)는 계측 두 값만으로 닫힌다.
...
> **퇴역(A-074, 오너 2026-09-25)**: 아래는 2026-09-08의 Figma 변경 기록이다. 여행 화면의 되돌리기 진입점은 2026-09-21에 빠졌고, 앱은 이 패널을 그리지 않는다.
...
| FCR-005 | `FE-502-T2`, `FE-301-T1`, `FE-401-T5` (`optimization-run.test.tsx`, `trip-screen.test.tsx`, `live.test.tsx`) | 실제 optimization/trip 데이터를 렌더한 뒤 route 문구·수치 부재 증명 완료. Live는 Figma가 '돌아가도 +N분'을 그렸던 장소 행(`listLiveAreaPlaces`)을 KO·EN 양쪽에서 렌더한 뒤 돌아가도·+N분·도보·우회·km(영문 대응 포함) 부재를 증명한다(`FE-401-T5`). 예전 근거였던 `FE-401-T1` case는 영어 UI의 권역 목록만 봤다 |
| FCR-006 | `FE-105-T1` (`profile.test.tsx`, `shell.spec.ts`) | A-075로 대체. 프로필 `TEST` 표기가 inert이고 route·요청 0건임을 증명한다. 로그인 흉내 /sign-in은 로그인·인증 요청 0건이다(`sign-in.test.tsx`) |
...
| FCR-012 | `FE-401-T1`·`T2`·`T3` (파일은 FCR-008 행과 같다), `FE-403-T2` (`live.test.tsx`), `FE-403-T3` (`live-replay-matrix.spec.ts` — 게이트, `live-map-sheet.mock.spec.ts`, `live-accessibility.mock.spec.ts`) | map OFF·list-first와 default/loading/empty/error/unavailable, keyboard·reflow의 증거 완료. 360·180px·ko/en·keyboard와 reduced motion은 게이트에서도 잰다(`live-replay-matrix.spec.ts`가 승인 fixture를 `page.route`로 준다). Figma sheet geometry·area-level 출처와 composed browser 승인은 대기 |
...
| FCR-015 | `FE-505-T1`·`T2` (`applied-panel.test.tsx`, `applied-revert.test.ts`, `trip-applied-panel.test.tsx`) | 퇴역(A-074). 컴포넌트 단위 증명은 남아 있지만 앱이 패널을 mount하지 않는다. 게이트는 `applied-panel.spec.ts`로 그 부재를 지킨다 |

따라서 Live와 무관한 구현과 Vitest 증거까지 완료됐다. 전체 종료는 종료 조건 5의
Storybook/Playwright ID 연결, `FCR-008`·`FCR-012`와 `FCR-011`의 Live 절,
그리고 이 문서가 별도로 요구하는 PM 범위·문구 승인을 기다린다. 대기 중인 조건을
별도 이슈로 명시적으로 이관하기 전에는 이 표만으로 #13을 닫지 않는다.

**`FCR-005`의 Live 절은 풀렸다.** 그 절이 기다리던 것은 Figma가 `돌아가도 +8분`을 그렸던
Live 장소 행에서 경로 수치가 없다는 증명이었다. 예전 근거인 `FE-401-T1` case는 영어 UI의
권역 목록만 봤다. `FE-401-T5`(`live.test.tsx`, 0b891ed0)가 `listLiveAreaPlaces`가 채운
장소 행을 KO·EN 양쪽에서 렌더한 뒤 돌아가도·+N분·도보·우회·거리(영문 대응 포함)의 부재를
단언한다. 그 commit이 잰 변이(한국어 우회·영어 detour·거리를 행에 넣기)는 각각 `FE-401-T5`
두 case를 빨갛게 했다. 위 표의 `FCR-005` 행이 이 증거를 적고 있다. `FCR-005`에 남는 것은
다른 행과 같은 종료 조건 5와 PM 승인뿐이다.
...
위 표에서 두 FCR의 대기 항목(fixture 승인, composed browser 증거, Figma sheet geometry·area-level 출처)은 그 세 이슈에서 닫는다. 이 이관은 두 FCR에 대한 것이다. `FCR-011`의 Live 절과 PM 범위·문구 승인은 옮기지 않았다(`FCR-005`의 Live 절은 위에서 풀렸다).

두 행의 test 파일은 ID별로 적었다. 예전에는 ID와 파일을 한 괄호에 묶어, `FE-402-T2`·`FE-403-T2`가 mock spec에도 있는 것처럼 읽혔다. 실제로는 둘 다 `live.test.tsx`에만 있다. 반대로 `FE-403-T3`는 `live.test.tsx`에 없고 두 mock spec에 있다 — 그리고 R4b부터는 게이트에서 도는 `live-replay-matrix.spec.ts`에도 있다(reduced motion 포함). 세 파일에서 `FE-40[1-3]-T[0-9]+`를 찾아 확인했고, 게이트 spec은 따로 확인했다.
