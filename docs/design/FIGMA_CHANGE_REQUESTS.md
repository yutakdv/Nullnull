...
| FCR-035 | P0 major | 혼잡 단계 척도가 정본 넷에서 불일치한다. `CrowdBar` 인스턴스(`438:3178`)는 칸이 **5개**이고 같은 화면에 `5 · 매우 혼잡`(`438:3192`)이 `4 · 혼잡`(`438:3179`)과 나란히 있는데, `COMPONENT_CATALOG.md`의 `CrowdLevel` 행은 `1~4단계`, FCR-004가 확정한 어휘는 `1·매우 여유 … 4·혼잡`, `419:2757`은 `4단계 중 4번째`, 구현은 `STEPS = 4`다. 계약의 `ordinalLevel`은 `type: [string, "null"]`에 enum·min/max·description이 없어 어느 쪽도 고정하지 않는다 | 척도를 4단계와 5단계 중 하나로 확정하고, 확정한 쪽으로 (a) `CrowdBar` 칸 수 (b) 화면 문구(`438:3179`·`438:3192`·`419:2757`) (c) `COMPONENT_CATALOG.md`의 `1~4단계` (d) 계약 `ordinalLevel`의 enum을 **같은 변경에서** 맞춘다. 넷 중 하나만 고치면 나머지 셋이 남아 같은 자리에서 다시 어긋난다. 그때까지 FE는 단계를 렌더하지 않고 현재 동작을 유지한다(FCR-030 선례). 수치를 임계값으로 나눠 단계를 합성하지 않는다 — 그 임계값은 아무도 재지 않은 것이고 불변식 8이 금지한다 | FE / BE·AI·PM | 오너 결정 · **5단계** (2026-09-19, Figma `CrowdBar` 5칸을 정본으로) · (b)(c)는 FE, (d) 계약 enum은 BE가 같은 흐름에서 맞춤 대기 |
| FCR-036 | P0 major | 예보 미보유 장소의 사유 문구가 없다. 예보는 운영자가 적재한 장소에만 있어 검색 결과 대부분이 예보 없음으로 오고, 계약은 그것을 `state: UNAVAILABLE` + `unavailableReason: NO_COVERAGE`로 보낸다. 받을 자리는 `CrowdLevel`의 `unavailableReason`에 이미 있으나 **shipped caller가 0건**이고, 그 문구를 가진 곳은 `CrowdLevel.stories.tsx`의 `관측 권역 밖이에요` 하나뿐이다 — `messages.ts`에 없는 storybook fixture의 문자열이라 i18n을 타지 않고 영어가 없다 | `NO_COVERAGE`일 때 `unavailableReason`에 들어갈 문구를 ko/en으로 정의한다. **재시도·새로고침을 권하지 않는** 문구여야 한다 — 이 장소는 나중에 다시 봐도 예보가 없으므로 `UNAVAILABLE`(일시적 부재)과 같은 문구면 일시적 장애로 읽힌다. 아울러 그 상태에서 혼잡 블록을 숨길지 자리를 남기고 사유만 보일지 정한다(`438:3158` 카드는 `CrowdBar`+문구+`Tag`+출처가 한 행이라 숨기면 행 높이가 바뀐다). 그때까지 FE는 FCR-016 선례대로 계약 근거만으로 선구현하고 Figma 확정 시 교체한다 | FE / BE·AI·PM | Open · 디자인 부재 (#105 batch 검토 중 발견 2026-09-19) |
