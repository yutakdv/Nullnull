...
| FCR-039 | P0 major | S02-4B 필수 방문(`438:3158`)에 '여행은 만들어졌는데 고른 장소 일부를 담지 못했다'는 상태의 frame이 없다. #185로 `이대로 채우기`가 `createTrip` 뒤 장소마다 `addTripCandidate`(`mustVisit: true`)를 보내게 되었고, 이 N+1 요청은 한 transaction이 아니라(불변식 5) 일부만 저장되는 상태가 실제로 생긴다 | 같은 step의 담은 곳 목록 아래에 alert 문구 `여행은 만들었어요. N곳을 담지 못했어요: 이름, 이름` / `Your trip was created, but N of your places couldn't be saved: …`를 두고, 하단 bar를 `다시 시도`(실패한 장소만, 처음과 같은 key로 같은 여행에)·`여행으로 가기`로 바꾼다. 이 상태에서는 `이대로 채우기`·`건너뛰기`·이전 단계를 두지 않고 담기·빼기를 막으며, 나타날 때 문구를 bar 위로 보이게 하고 focus를 `다시 시도`로 돌린다(`FE-103-T30`~`T47`). 위치·문구·버튼 배치는 Figma가 그릴 때까지의 FE placeholder이고 그리면 그 시안을 따른다 | FE / PM | 구현 · Figma node 대기 (FE-103 #185, 2026-09-27) |

FCR 번호는 기존 참조를 보존한다. FCR-010~015는 09-06 PR #6 계약 검토에서, FCR-016~028은 09-06 직접 canvas 대조와 API 검토에서, FCR-029는 FE-103, FCR-030은 FE-301, FCR-031은 FE-303, FCR-032는 FE-306, FCR-033은 FE-307 범위 확정 중 계약·컴포넌트 규칙 대조에서, FCR-034는 FE-505 구현 중 Figma frame 실측에서, FCR-037은 FE-306 진입점 복구 중, FCR-039는 FE-103 필수 방문 장소 저장(#185) 중 추가했다. 등록은 문서 기준선이며 디자인 수정 완료를 뜻하지 않는다.
