...
| FCR-020 | P0 blocker | 438:3108 동행인/스타일 chip에 canonical code/weight 없음 | 지원 dictionary·선택 규칙·KOEN label·중복/빈 선택; PM-006 | FE / BE·AI·PM | Closed · 어휘 확정(2026-09-12, [증거](#fcr-020-증거), [#154](https://github.com/yutakdv/Nullnull/issues/154) CLOSED). 서버는 `InterestVocabularyContractTest`(`BA-030-T1`)가 계약 `x-nullnull-interest-codes`와 `InterestVocabulary`를 양방향으로 고정하고, 화면 문구는 `message-key-coverage.test.ts`가 `INTEREST_GROUPS`의 모든 code에 KO/EN label이 있는지 본다 |
...
| FCR-024 | P0 major | feed/post 작성자·하트 수·반응 상태와 read schema 불일치 | **화면 범위 확정**(#163 — 작성자·하트·반응을 P0 화면에서 제거). `FR-FED-04`는 계측(`IMPRESSION`/`OPEN`)만으로 닫는다 — 두 값은 버튼이 필요 없어 화면 변경 없이 기록된다. `LIKE`/`DISLIKE`는 누를 control이 없으므로 P1. **`HIDE`는 P0 미구현**: 복구 진입점이 저장소 전체에 0건이라([증거](#fcr-024-증거)) 지금 구현하면 숨긴 글이 돌아올 길이 없다; PM-011 | FE / BE·AI·PM | Closed · 화면 범위 확정 · 계약·구현 완료 · Figma 반영 완료 (2026-09-18, [증거](#fcr-024-증거), [#163](https://github.com/yutakdv/Nullnull/issues/163) CLOSED). 단서: 이 행이 `FR-FED-04`를 닫는 수단으로 적은 `IMPRESSION`·`OPEN` 계측은 아직 배선되지 않았다 — `recordFeedFeedback`를 부르는 production 코드가 0건이고 `feed.test.tsx`의 `FE-201 #163` case가 요청 0건을 고정한다. 그것은 이 FCR의 화면 범위가 아니라 FE 코드 작업으로 남는다 |
