...
| FCR-035 | P0 major | 혼잡 단계 척도가 정본 넷에서 불일치한다. `CrowdBar` 인스턴스(`438:3178`)는 칸이 **5개**이고 같은 화면에 `5 · 매우 혼잡`(`438:3192`)이 `4 · 혼잡`(`438:3179`)과 나란히 있는데, `COMPONENT_CATALOG.md`의 `CrowdLevel` 행은 `1~4단계`, FCR-004가 확정한 어휘는 4단계까지였다 | 척도는 5단계로 고정한다. `CrowdBar`·KO/EN 화면 문구·component catalog·계약 `ordinalLevel`을 같은 척도로 맞추고 수치에서 단계를 합성하지 않는다 | FE / BE·AI·PM | Closed · `CrowdLevel` 5칸과 `1 · 매우 여유`~`5 · 매우 혼잡` KO/EN 문구, component catalog, 계약 `^[1-5]$`가 일치함 (2026-09-20). 서울 변형은 새 FCR 없이 A-060(오너 2026-09-20)이 정한다: 서울의 네 단계는 5칸 중 1~4칸에 놓이고 5칸째는 비운 칸으로 그리며(`FE-403-T4`), 단계 문구는 서울 어휘 `여유`·`보통`·`약간 붐빔`·`붐빔`이고, 막대 이름은 칸 수가 아니라 발행 단계 수로 `4단계 중 N번째`라고 읽는다 — 앱 안에서는 `서울 혼잡도 4단계 중 N번째`(`COMPONENT_CATALOG.md`의 `CrowdLevel` 행). 증거는 `FE-403-T4`·`FE-403-T5`다: 컴포넌트 단위의 칸과 이름은 `data-components.test.tsx`가, 앱 안의 KO·EN 이름(`서울 혼잡도 4단계 중 N번째`·`Seoul crowd level N of 4`)과 비운 5칸째는 게이트의 `live-replay-matrix.spec.ts`가 잰다 |
