...
| FCR-006 | P0 blocker | S14 `422:2925`가 `로그인하면 일정을 저장할 수 있어요`와 활성 login affordance를 노출 | `이 기기의 익명 세션에 저장돼요`처럼 실제 보존 방식을 설명하고 login은 disabled `준비 중`; 요청 0건 | FE / BE·AI·PM | A-075(오너 2026-09-25)가 대체: /sign-in의 공모전 테스트 계정 prefill(브라우저 안 대조, 요청 0건)과 프로필 `TEST` 표기를 유지한다. 이 행의 `준비 중` 요구는 적용하지 않는다. 이전 기록: Figma 수정 완료 · 검토 대기 (2026-09-07, [증거](#fcr-006-증거)) |
...
| FCR-006 | `FE-105-T1` (`profile.test.tsx`, `shell.spec.ts`) | A-075로 대체. 프로필 `TEST` 표기가 inert이고 route·요청 0건임을 증명한다. 로그인 흉내 /sign-in은 요청 0건이다(`sign-in.test.tsx`) |
