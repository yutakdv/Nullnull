...
- PR의 OpenAPI breaking diff는 `origin/main:docs/api/openapi.yaml`과 checkout된 계약을 비교한다. [oasdiff action](https://github.com/oasdiff/oasdiff-action/tree/9c0494cfee8b8fcc9fb383ed2d5d3fbdae169b93/breaking) v0.1.15 SHA와 WARN 실패 기준을 고정한다. spec upload와 PR comment는 끈다. push/workflow_dispatch에는 비교하지 않는다. 의도된 계약 정정은 `docs/api/oasdiff-warn-ignore.txt`의 **정확한 메시지 한 줄**로만 면제되고 [승인 예외 등록부](../api/BREAKING_CHANGE_EXCEPTIONS.md)에 이유·승인자·추적 이슈가 있어야 한다. `scripts/tests/test_oasdiff_exceptions.py`가 미등록·미정당화·stale 행을 실패시킨다. 이것은 검사를 끄는 것이 아니다 — 면제되지 않은 breaking 변경은 그대로 실패한다(실측 확인).
