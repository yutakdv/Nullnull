...
| REC-LLM-04 | `AI_PROVIDER` 기본값·미지원 이름·자격 증명 없는 named provider | 설명은 template 이고 model 귀속 표기 0, 미지원 이름과 빈 자격 증명은 startup 실패 | settings + wiring |
| REC-LLM-06 | 모델 adapter 의 요청 shape·실패 매핑·키 취급 | template 만 전달, 실패는 명명된 provider error, 키는 header 밖(url·body·응답·log·예외 문구)에 없음 | injected transport + urlopen monkeypatch |
| REC-LLM-05 | 호출자가 준 어휘 밖의 code·중복·범위 밖 weight 해석 | 어휘가 말할 수 없는 해석은 거부, 정규화·보정 0 | apps/ai unit |
| REC-LOC-01 | 같은 facts 로 렌더한 KO/EN 설명 문장 | 두 문장이 같은 수·같은 slot(월·일·시각)을 말한다 | apps/ai unit |
...
- `apps/web/e2e/session.integration.spec.ts`는 `API_INTERNAL_BASE_URL`의 실제 API를 직접 호출하는 Playwright transport 검사다. Compose 내부 HTTP에서는 Secure cookie를 명시 전달한다. 브라우저 Secure cookie 수락이나 아직 없는 세션 UI를 검증했다고 쓰지 않는다. 기존 `shell.spec.ts`의 keyboard/focus 검사는 계속 실행한다.
...
- `apps/web/e2e/session.integration.spec.ts`는 실제 API에서 삭제 접수·정확한 replay·상태 조회·revoked 접근 차단을 검사한다. S14 UI의 확인 dialog, memory-only token 보관, polling 진행/부분 실패/만료 화면은 Frontend 검수 항목이다.
...
- PR의 OpenAPI breaking diff는 `origin/main:docs/api/openapi.yaml`과 checkout된 계약을 비교한다. [oasdiff action](https://github.com/oasdiff/oasdiff-action/tree/9c0494cfee8b8fcc9fb383ed2d5d3fbdae169b93/breaking) v0.1.15 SHA와 WARN 실패 기준을 고정한다. spec upload와 PR comment는 끈다. push/workflow_dispatch에는 비교하지 않는다. 의도된 계약 정정은 `docs/api/oasdiff-ignore.txt`의 **정확한 메시지 한 줄**로만 면제되고 [승인 예외 등록부](../api/BREAKING_CHANGE_EXCEPTIONS.md)에 이유·승인자·추적 이슈가 있어야 한다. `scripts/tests/test_oasdiff_exceptions.py`가 미등록·미정당화·stale 행을 실패시킨다. 이것은 검사를 끄는 것이 아니다 — 면제되지 않은 breaking 변경은 그대로 실패한다(실측 확인).
