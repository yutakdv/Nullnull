...
/**
 * The signing material for listTrips cursors.
 *
 ...
 */
@Component
public final class TripCursorProperties {

    private static final String HMAC = "HmacSHA256";
    private static final String KEY_ID = "trip-v1";
    ...
    public static final String CONTEXT = "listTrips";

    private final byte[] cursorSecret;
    private final SignedCursorCodec cursorCodec;

    public TripCursorProperties(@Value("${NULLNULL_CURSOR_SECRET:}") String configuredSecret,
            Environment environment) {
        String secret = Objects.requireNonNull(configuredSecret, "configuredSecret").trim();
        if (secret.isEmpty()) {
            if (!developmentProfile(environment)) {
                throw new IllegalArgumentException("NULLNULL_CURSOR_SECRET is required to sign trip cursors");
            }
