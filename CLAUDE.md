...
- 가입 없는 익명 session이 P0다. 로그인 화면(`/sign-in`)은 공모전 테스트 계정을 미리 채운 흉내이고, 브라우저 안에서만 대조하며 인증 요청을 보내지 않는다. 프로필은 'TEST' 표기를 보이고 로그인 컨트롤을 두지 않는다(A-075). 실제 계정 로그인은 P1이다.
...
- `bash scripts/integration-test.sh`는 `python3 scripts/verify_target_stack.py`와 함께 실행한다. marker `.nullnull-target-stack`은 **이미 있다**(같은 파일 위쪽 baseline 줄, tree로 확인). marker 부재로 인한 exit 1은 더 이상 이 저장소의 상태가 아니다 — 이 줄이 그 전제를 들고 한동안 낡아 있었다.
