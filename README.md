...
```bash
.venv/bin/python -m pytest                        # 테스트 (66개)
...
- 백엔드가 꺼져 있으면 배포된 웹앱은 자동으로 목업 데이터(오프라인 데모)로 표시됩니다.
...
| GET | `/api/spots/{id}/congestion?date=&time_slot=` | 널널도(F3): 5단계 뱃지, ±5일 요일 비교, 오전/오후/저녁 비교, 팁. **date가 오늘~+30일 밖이면 400** |
| GET | `/api/spots/{id}/alternatives?date=&time_slot=&themes=&limit=` | 대안지(F4·F6): AlternativeScore 정렬, 감소율·이동시간·유사도·추천이유·점수 분해, **노출 로그 기록(F8)** |
| POST | `/api/courses` | 코스 생성(F5): 동선 정렬, 근거 저장, **선택 로그 기록(F8)** |
...
가중치 백테스트(기획서 9-5)는 `weights.yaml` 수정 → `python -m app.batch.daily` 재계산으로 수행합니다.
